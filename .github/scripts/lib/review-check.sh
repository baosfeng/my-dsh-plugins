...
review_finish() {
  local report="$1" suggest="$2" history="${3:-}" body overall
  body="$(mktemp)"
  cp "$report" "$body"
  local pass fail unknown
  ...
  {
    printf '## 结论\n\n%s\n\n' "$overall"
    printf '统计：通过 %s 项 / 不通过 %s 项 / 未能判定 %s 项\n\n' "${pass:-0}" "${fail:-0}" "${unknown:-0}"
  ...
  rm -f "$body"
  # 硬约束：报告 ≤30 行，超限即截断并指向 artifact（工程效率规范第十三节）
  node -e '
    const fs = require("fs")
...
# review_unknown <报告文件> <检查名> <原因>：明确「没真正完成检查」时使用，严禁写 ✅。
review_unknown() {
  printf '#### %s\n\n**结论**：未能判定（%s）\n\n' "$2" "$3" >>"$1"
}

# review_check_scope <报告文件> <检查名> <摘要 kind> <门禁路径正则> <命令...>
# 用于「门禁口径 vs 全量」不一致的检查（当前仅圈复杂度）：
#   结论三态只由**门禁口径**（匹配正则的路径）决定；全量数字作为「提示（非门禁）」，不参与 outcome。
review_check_scope() {
