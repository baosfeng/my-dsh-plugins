...
4. **分配优先，leader 不亲力亲为**：一切可独立任务（开发/修复/验证/排查/文档/配置/调研）优先派子 agent（fork 池隔离，见 dsh-github-triage skill）；**验证、翻日志、跑命令尤其要派出去**（亲力亲为浪费 leader 上下文）。leader 只做拆解、决策、验收、汇报，上下文只留决策信息（agent id、验收标准、进度）。**唯一例外**：特别简单、且**派发开销 > 自做开销**的小事（判据就是这一句），如改一行配置、回一句确认。
...
9. **效率即质量**：阻塞性环节零容忍——能流程化/自动化/并行/流水线化的必须落地，每项优化都要有 before/after 实测数字且不得削弱门禁；派任务须同时给**验收判据**与**停止条件**（详见 docs/开发指南/工程效率规范.md）。
10. **外部能力吸纳**：引入外部能力（skill/工具/方法论/组件）遵循四条——**引入通用的**（无场景的不引入）；**对方更好就采纳**（即使已有类似，更好的增量也要接受）；**引入后确保发挥作用**（引用 + 交叉引用 + 实际流程触发，不放在那里）；**接纳有条件**（符合项目风格：精简/风格统一/体积控制/核心价值保留）。
