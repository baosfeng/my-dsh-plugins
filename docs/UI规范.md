...
- 共享 parts 位于 **`plugins/dsh-shared/client-parts/`**（当前 4 个：`icons.part.js` / `style-tag.part.js` / `dom-scanner.part.js` / `markdown-fallback.part.js`，见下「共享部件清单」）。
...
| `markdown-fallback.part.js` | `installMarkdownViewFallback({ require, createElement, labels, codeLabels, fallbackAttribute, fallbackClassName?, external?, platformModule? })`：Markdown 渲染三级回退（外部内核 `dsh-md-render.MarkdownView` → 宿主平台 `@deepseek-ai/dsh-client-ui-primitives.MarkdownText` → 消费方 `<pre>` 兜底）+ 组件可用性判定（React 语义，兼容 memo/forwardRef 对象）。文案与 DOM 标记由消费方注入（共享件不含硬编码中文） | dsh-my-plugin-manager（issue #299 第一阶段）；dsh-think-zh-expand 待 #295 合并后接入 |
