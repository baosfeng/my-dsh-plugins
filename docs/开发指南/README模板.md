...
> **新插件 / 修改 README 时按此模板对齐**；`release.mjs` 已强制「效果截图」门禁（README 必须引用 `assets/` 下的真实截图——相对路径或 unpkg 绝对 URL，否则发版失败）；确无用户可见 UI 的插件按 `package.json` 的 `dsh.ui=false` + `dsh.uiReason` 显式豁免（见「相对路径与图片约定」）。
...
- **无 UI 插件的豁免**：确无用户可见产物（无 client / 无面板 / 无设置页 / GUI 中无可观察效果）的插件，在 `package.json` 声明 `"dsh": { "ui": false, "uiReason": "<为什么没有可截图的产物>" }` 即豁免发版门禁；`dsh.ui=false` 与 `dsh.client` 互斥、缺 `uiReason` 会被门禁拒绝。README 中仍要向读者显式说明「无 UI / 不产生界面截图」，并给出可复核的生效证据（日志 / 探针 / 接口）。
