---
title: 侧边栏原生 API 迁移指南
description: 宿主原生侧边栏扩展点（slots / sidebarRightTabs / documentPreviews / sidebarRight）的精确语义、迁移映射、DOM 契约与降级要求
---

# 侧边栏原生 API 迁移指南

> ⚠️ **何时阅读：** 新增页签 / 文件预览器 / 浮层 / 设置页时；把插件侧边栏能力从第三方 `dsh-better-sidebar` 迁到宿主原生扩展点时。
...
- ⚠️ **会话表面未挂载时调用 `openTab` 会 throw**（`sidebarRight: no session surface is mounted`）——自动打开必须在 `ctx.effect` 内触发并监听会话挂载后重试。**失败必须可见**（见 §4），不能吞异常。
...
- 开关的**权威存储**改为插件自己的配置表（server 端路由 + 持久化），不再依赖宿主 `prefs.pluginSettings`（旧开关状态一次性丢失）。
- UI 用官方组件库（宿主 0.1.5-rc.1 实测导出 **123** 项，见 [官方 UI 组件库](官方UI组件库.md)）。
...
移除 better-sidebar 后要同步：根 `package.json` 的依赖行、`knip.json` 的 `ignoreDependencies`、锁文件（清理后 `npm ls dsh-better-sidebar` 必须无输出）、`skills/dsh-plugin-development/references/better-sidebar-api.md` 与相关文档表述。client 源码只用 Cordis 服务名，不 require 这些包，故编译期无影响；要用官方组件库时它由宿主 staticModules 运行时提供，无需 npm 依赖。
...
**未验证边界（诚实记录）**：多会话切换下页签归属与会话隔离；`shell.overlay` 拿不到 session props（需把 sessionId 存进预览目标）；真实 HMR 重载下的注册去重；`priority` 多档与匹配模式长度的竞争排序。
