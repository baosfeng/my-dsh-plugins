 ...
 */
export function createPersist(options, stateOf) {
    const logger = options.ctx.logger;
    const guard = {
        file: options.file,
    ...
    return {
        file: options.file,
        appender: handle,
        append: (record) => handle.append(JSON.parse(eventLine(record))),
...
function throttleFor(file, bytes, guard) {
    if (bytes <= SNAPSHOT_THROTTLE_MIN_BYTES)
        return 0;
...
function guarded(guard, bytes, now) {
    if (bytes > guard.maxBytes) {
        guard.logger?.warn(LOG_PREFIX + ' compact rejected (' + bytes + 'B > maxBytes=' + guard.maxBytes + '): ' + guard.file);
        return true;
    }
 ...
 * 并告警（不静默）。
 */
async function writeSnapshot(handle, state, guard) {
    const lines = snapshotLines(state);
    const bytes = Buffer.byteLength(lines.join('\n') + '\n', 'utf8');
    if (guarded(guard, bytes, Date.now()))
        return false;
...
export function readAppenderStats(appender) {
...
function defaultAppender(file, options) {
    return jsonlAppender(file, options);
}
