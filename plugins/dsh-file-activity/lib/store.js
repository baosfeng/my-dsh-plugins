 ...
 */
function compactLinesFor(stateBytes) {
...
function mergeEvicted(state, evicted) {
...
function applyToState(handle, sessionId, op, path, time) {
...
function enqueueFlush(handle, immediate) {
    if (immediate) {
        flushFacts(handle);
        return;
    }
...
function flushFacts(handle) {
    ...
    if (persist === null || facts.length === 0)
        return;
...
function dispose(handle) {
    ...
    if (persist === null)
        return;
    for (const fact of handle.facts.splice(0))
        persist.append(fact);
    handle.dirtyChain = handle.dirtyChain
        .then(() => persist.flush())
