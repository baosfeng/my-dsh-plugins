...
const opClass = (op: string): string =>
  op === 'create'
    ? 'dfa-op-create'
    : op === 'modify'
      ? 'dfa-op-modify'
...
const fileTitle = (abs: string, firstSeen?: number, lastSeen?: number): string => {
  const times: string[] = []
...
const countPills = (node: TreeNode): unknown => {
  const pills: unknown[] = []
  if (node.read > 0)
...
const NAME_BADGES: Record<string, string> = {
  makefile: 'makefile',
...
const badgeKeyOf = (name: string): string => {
...
const fileRow = (file: TreeNode, depth: number, onOpen: (path: string) => void): unknown =>
  createElement(
    'div',
    {
      key: file.abs,
...
const recentEntry = (entry: RecentEntry, onOpen: (path: string) => void): unknown =>
  createElement(
    'div',
    {
...
function clearSessionData(dataStore: DataStore<DataState>, sessionId: string): void {
