...
import type { ActivityStore, DshContext, ServerRequest, ServerResponse } from './types.js'
...
function mediaError(status: number, message: string): Error & { status: number } {
  const error = new Error(message) as Error & { status: number }
  error.status = status
  return error
}

/** createMediaHandler 的依赖：ctx + store + 信任围栏。 */
...
export function createMediaHandler({
  ...
  store,
  fence,
}: MediaHandlerOptions): (request: ServerRequest, response: ServerResponse) => Promise<void> {
 ...
 */
async function serveRecordedFile(
...
function assertMediaParams(sessionId: string | null, raw: string | null): asserts raw is string {
