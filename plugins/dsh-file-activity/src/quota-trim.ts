...
import {
  ...
  emptyEvicted,
  enforceSessionLimit,
  enforceSessionPathLimit,
  enforceTotalPathLimit,
  type EvictedStats,
  type QuotaLimits,
} from './quota.js'
...
/** 加载规整结果：{ state, trimmed, evicted }。 */
export interface TrimQuotaResult {
  state: ActivityState
  trimmed: boolean
...
/** 计数器规整：非法值归零、时间戳可选保留。 */
function sanitizeCounters(raw: Counters | null | undefined): Counters | null {
...
function normalizeRecent(raw: RecentEntry[]): RecentEntry[] {
...
function normalizeCounts(raw: Record<string, Counters> | null | undefined): {
...
function alignKnown(
  ...
  counts: Record<string, Counters>,
): { known: Record<string, number>; changed: boolean } {
  ...
  if (Object.keys(known).length !== Object.keys(counts).length) changed = true
  for (const path of Object.keys(counts)) {
    if (typeof known[path] === 'number') continue
...
function trimSession(
  ...
  evicted: EvictedStats,
  logger?: Logger,
): { session: SessionState; trimmed: boolean } {
  ...
  const session: SessionState = { known, counts, recent }
  const before = Object.keys(counts).length
  enforceSessionPathLimit(session, sessionId, limits, evicted, logger)
  const trimmed =
    recent.length !== rawRecent.length || dropped > 0 || changed || Object.keys(session.counts).length !== before
 ...
 */
export function trimToQuota(loaded: LoadedState, limits: QuotaLimits, logger?: Logger): TrimQuotaResult {
  ...
  for (const [sessionId, raw] of Object.entries(loaded.sessions ?? {})) {
    ...
    if (result.trimmed) trimmed = true
    sessions[sessionId] = result.session
  }
  const state: ActivityState = { version: 1, sessions, stats: { evicted } }
  const sessionsBefore = Object.keys(sessions).length
  enforceSessionLimit(state, limits, evicted, logger)
  if (Object.keys(state.sessions).length !== sessionsBefore) trimmed = true
  const pathsBefore = countPaths(state)
  enforceTotalPathLimit(state, limits, evicted, logger)
  if (countPaths(state) !== pathsBefore) trimmed = true
  if (evicted.sessions + evicted.paths + evicted.pathsTotal > 0) trimmed = true
