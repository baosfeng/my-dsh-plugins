...
import { createSession, defaultLimits, emptyEvicted, type QuotaLimits } from './quota.js'
...
interface SnapshotStats {
  evicted?: { sessions?: number; paths?: number; pathsTotal?: number }
}

/** 解析结果：会话表 + 快照统计 + 是否命中旧格式快照（整文件 JSON）。 */
...
export function parseLegacySnapshot(text: string): ParsedFile | null {
  let parsed: unknown
  try {
  ...
  if (doc.version !== 1 || doc.sessions === null || typeof doc.sessions !== 'object') return null
  return { sessions: doc.sessions as Record<string, unknown>, legacy: true }
}

/** 事件行（记录行）：`{"s","p","o","t"}`；非记录行返回 null。 */
function parseRecordLine(value: unknown): ParsedRecord | null {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return null
  const row = value as { s?: unknown; p?: unknown; o?: unknown; t?: unknown }
  if (typeof row.s !== 'string' || typeof row.p !== 'string') return null
  if (row.s === '' || row.p === '') return null
 ...
 */
export function parseStateFileText(text: string): { snapshot: Record<string, unknown>; records: ParsedRecord[] } {
  ...
  for (const entry of parseStateEntries(text)) {
    ...
    if (session !== null) snapshot[entry.sessionId] = session
  }
...
/** 状态文件的一行（按行序重建状态）。 */
type StateFileEntry = { kind: 'snapshot'; sessionId: string; data: unknown } | { kind: 'record'; record: ParsedRecord }
...
function toEntry(row: Record<string, unknown>): StateFileEntry | null {
  const data = row.d
...
function parseStateEntries(text: string): StateFileEntry[] {
...
/** 从文本中提取快照统计（历次淘汰计数）。 */
function extractSnapshotStats(text: string): SnapshotStats | undefined {
  const match = META_LINE.exec(text)
  if (match === null) return undefined
  try {
    const meta = JSON.parse(match[0]) as { stats?: unknown }
    if (meta.stats === null || typeof meta.stats !== 'object') return undefined
    return meta.stats as SnapshotStats
  } catch {
    return undefined
  }
}
...
function toKnown(raw: unknown): Record<string, number> {
  const known: Record<string, number> = {}
  if (!isPlainObject(raw)) return known
  for (const [path, at] of Object.entries(raw)) {
    if (typeof at === 'number' && path !== '' && !path.includes(NUL)) known[path] = at
  }
  return known
}

/** counts 表规整（非法 path / 非法计数器丢弃）。 */
function toCounts(raw: unknown): Record<string, Counters> {
  const counts: Record<string, Counters> = {}
  if (!isPlainObject(raw)) return counts
  for (const [path, value] of Object.entries(raw)) {
    if (path === '' || path.includes(NUL)) continue
    const counters = sanitizeCounters(value)
    if (counters !== null) counts[path] = counters
  }
  return counts
}

/** recent 列表规整（非法条目丢弃）。 */
function toRecent(raw: unknown): RecentEntry[] {
  const recent: RecentEntry[] = []
  if (!Array.isArray(raw)) return recent
  for (const entry of raw) {
    if (!isPlainObject(entry)) continue
    const path = entry.path
    if (typeof path !== 'string' || path === '' || path.includes(NUL)) continue
...
/** 会话数据规整（结构非法返回 null）。 */
function toSession(raw: unknown): SessionState | null {
  if (!isPlainObject(raw)) return null
  return { known: toKnown(raw.known), counts: toCounts(raw.counts), recent: toRecent(raw.recent) }
}

/** 计数器防御性读取（非法结构返回 null）。 */
function sanitizeCounters(raw: unknown): Counters | null {
  if (!isPlainObject(raw)) return null
  const num = (value: unknown): number => (typeof value === 'number' && Number.isFinite(value) ? value : 0)
  const counters: Counters = { read: num(raw.read), create: num(raw.create), modify: num(raw.modify) }
  if (typeof raw.firstSeen === 'number') counters.firstSeen = raw.firstSeen
  if (typeof raw.lastSeen === 'number') counters.lastSeen = raw.lastSeen
  return counters
}
...
export async function loadState(
  file: string,
  limits: QuotaLimits = defaultLimits(),
  logger?: Logger,
): Promise<{ state: ActivityState; trimmed: boolean; evicted: ReturnType<typeof emptyEvicted>; legacy: boolean }> {
  let text = ''
  try {
...
/** 旧格式（整文件 JSON 快照）→ 会话表。 */
function sessionsOfLegacy(legacy: ParsedFile): Record<string, SessionState> {
  const sessions: Record<string, SessionState> = {}
  for (const [sessionId, raw] of Object.entries(legacy.sessions)) {
    const session = toSession(raw)
    if (session !== null) sessions[sessionId] = session
  }
  return sessions
}

/** JSON Lines → 会话表（快照行建立基线，事件行按序重放 = 运行时状态的精确重现）。 */
function sessionsOfJsonl(text: string): Record<string, SessionState> {
  const state = createState()
  for (const entry of parseStateEntries(text)) {
    if (entry.kind === 'snapshot') {
      const session = toSession(entry.data)
      if (session !== null) state.sessions[entry.sessionId] = session
      continue
    }
...
function mergeSnapshotStats(
  result: ReturnType<typeof trimToQuota>,
  stats: SnapshotStats | undefined,
): ReturnType<typeof trimToQuota> {
  if (stats === undefined) return result
  result.evicted = {
...
function isValidRecordTarget(sessionId: unknown, path: unknown): boolean {
  return (
    typeof sessionId === 'string' && sessionId !== '' && typeof path === 'string' && path !== '' && !path.includes(NUL)
  )
}
...
export function foldRecord(
  ...
  op: string,
  time?: number,
): { session: SessionState; created: boolean; newPath: boolean } | null {
  if (!isValidRecordTarget(sessionId, path)) return null
