...
import type { ActivityStore, DshContext, LoadedState, StoreHandle } from './types.js'

/** Build the per-apply store: { state, record, schedulePersist, dispose }. */
export function createStore(ctx: DshContext): ActivityStore {
...
function onLoaded(handle: StoreHandle, loaded: LoadedState): void {
