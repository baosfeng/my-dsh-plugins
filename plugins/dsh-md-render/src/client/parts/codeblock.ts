...
function renderTokens(tokens: Token[]): unknown[] {
  const out: unknown[] = []
...
function shouldHighlight(lang: string, lines: string[]): boolean {
...
function renderCodeHead(lang: string, bottomCopy: boolean): unknown {
...
function renderCodeBlock({ key, lang, code }: { key: string; lang: string; code: string }): unknown {
  ...
  const body = renderCodeCells(code, lang, lines, highlight, renderOptions.lineNumbers as boolean)
  // issue #146：主题仅作用于实际高亮的代码块——关闭 syntaxHighlight
  // / 未知语言 / 超长跳过高亮时无 data-theme，保持 DSH 语义 token 默认
  // 样式（主题不影响纯文本代码块，开关语义不回归）。
  const blockProps: Record<string, unknown> = { key, className: 'md-code-block' }
