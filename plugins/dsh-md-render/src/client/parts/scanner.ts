...
function scanNode(seen: Set<Node>, node: Node): void {
  if (
    ...
    ((node as Element).matches('div.tzx-md') || (node as Element).matches('div.md-table-wide'))
  ) {
    scanContainer(seen, node as Element)
    return
  }
  if (node && typeof (node as Element).querySelectorAll === 'function') {
    for (const c of (node as Element).querySelectorAll('div.tzx-md, div.md-table-wide')) {
