...
- **默认提示模型主动画图**：host 半默认向系统提示词注入一段简短能力说明，因此用户不写「mermaid」字样（如只说「画一个登录流程图」）时模型也会主动输出 mermaid 代码块；可配 `injectPrompt: false` 关闭，也可在 `设置 → 插件 → Mermaid 渲染` 里直接开关（见下节）。
...
**可视化编辑**：`设置 → 插件 → Mermaid 渲染` 有同名开关（开通即注入 / 关闭即不注入）。保存走 `PUT /mermaid-render/api/config`，写回 profile 层 `cordis.patch.yml` 里本插件的配置行（上面那个 `- id: mermaid-render`），重启不丢；保存后当前进程当即撤销/注册系统提示词段（**热生效，无需重启**）。写入非法值（非布尔）一律按默认 `true` 落盘——不会把脏值写进配置文件。
