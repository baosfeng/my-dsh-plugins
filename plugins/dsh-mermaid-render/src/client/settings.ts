...
function MermaidSettingsLoadError(props: MermaidSettingsErrorProps): unknown {
  ...
  return createElement(
    ...
    createElement('div', { className: 'dsh-mermaid-render-settings-error' }, '配置加载失败 / Failed to load config'),
    createElement('div', { className: 'dsh-mermaid-render-settings-status' }, hint),
    createElement(
      'div',
      { className: 'dsh-mermaid-render-settings-actions' },
      createElement('button', { className: 'dsh-mermaid-render-settings-btn', onClick: props.onRetry }, '重试 / Retry'),
    ),
  )
}

/** 注入开关说明（灰字）：默认开，关闭只影响「主动引导」，不影响渲染本身。 */
const MERMAID_SETTINGS_HINT =
  '默认开启：模型在用户没写出「mermaid」字样时也会主动输出 ```mermaid 代码块。关闭后已有 mermaid 代码块照常渲染，只是不再主动引导 / When on, the model proactively emits ```mermaid blocks; when off, existing blocks still render — the model is just no longer nudged.'
...
function MermaidSettingsActions(props: { saved: boolean; failed: boolean; onSave: () => void }): unknown {
  return createElement(
    'div',
    { className: 'dsh-mermaid-render-settings-actions' },
    createElement('button', { className: 'dsh-mermaid-render-settings-btn', onClick: props.onSave }, '保存 / Save'),
    props.saved ? createElement('span', { className: 'dsh-mermaid-render-settings-saved' }, '已保存 / Saved') : null,
    props.failed
...
function MermaidRenderSettingsView(): unknown {
  ...
  if (loading) {
    return createElement(
      'div',
      { className: 'dsh-mermaid-render-settings' },
      createElement('div', { className: 'dsh-mermaid-render-settings-status' }, '加载中… / Loading…'),
    )
  }
