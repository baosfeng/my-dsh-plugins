...
export function createConfigState(config?: { injectPrompt?: unknown }): MermaidConfigState {
 ...
 */
export function normalizeConfigPayload(payload: unknown): MermaidConfigState | undefined {
  if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) return undefined
  return { injectPrompt: (payload as { injectPrompt?: unknown }).injectPrompt !== false }
}

/** 写回 profile patch 文件（原子写）；失败向上抛，调用方据此回错误码且不动内存。 */
export async function persistConfig(next: MermaidConfigState): Promise<void> {
