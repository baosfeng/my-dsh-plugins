...
export function estimateBlocks(blocks: unknown[]): number {
  ...
  for (const block of blocks) {
    ...
    switch (obj.type) {
      case 'text':
      case 'reasoning':
        tokens += estimateText(obj.text as string)
        break
      case 'tool-call':
        tokens += estimateText(obj.name as string) + estimateText(obj.arguments as string) + BLOCK_OVERHEAD
        break
      case 'tool-result':
        tokens += estimateBlocks(obj.content as unknown[]) + BLOCK_OVERHEAD
        break
      default:
