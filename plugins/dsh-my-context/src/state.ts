...
import type { BoundedList, BoundedMap } from 'dsh-shared'
import {
  MAX_ALERTS_PER_SESSION,
  MAX_OVERFLOWS_PER_SESSION,
...
export interface SessionState {
  ...
  requests: BoundedList<unknown>
  header: { system: string; tools: unknown[]; systemTokens: number; toolsTokens: number }
  alerts: BoundedList<unknown>
  overflows: BoundedList<unknown>
  updatedAt: number
}

/** 全局状态：bySession 为有界 Map（LRU，上限 MAX_SESSIONS）。 */
export interface ContextState {
  version: number
  bySession: BoundedMap<string, SessionState>
}

/** 初始空状态（bySession 有界：会话数超上限淘汰最久未使用）。 */
export function createState(): ContextState {
  return { version: 1, bySession: boundedMap<string, SessionState>({ maxSize: MAX_SESSIONS }) }
}
...
export function createSession(sessionId: string): SessionState {
  return {
    ...
    // 不能作为"当前上下文占用"。
    lastPromptTokens: 0,
    requests: boundList<unknown>({ maxSize: MAX_REQUESTS_PER_SESSION }),
    header: { system: '', tools: [], systemTokens: 0, toolsTokens: 0 },
    alerts: boundList<unknown>({ maxSize: MAX_ALERTS_PER_SESSION }),
    overflows: boundList<unknown>({ maxSize: MAX_OVERFLOWS_PER_SESSION }),
    updatedAt: 0,
  }
}
