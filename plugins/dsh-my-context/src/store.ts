 ...
 * 不引入新的失败面。
 */
export async function whenReadyOf(store: unknown): Promise<void> {
  const ready = (store as { whenReady?: unknown } | null | undefined)?.whenReady
...
export function createStore(ctx: DshContext): Record<string, unknown> {
  const store = { state: createState() } as Record<string, unknown>
  ...
  const handle: PersistHandle & { seq: number } = {
    ctx: ctx as PersistHandle['ctx'],
    file: stateFile(),
  ...
  store.updateHeader = (sessionId: string, header: Record<string, unknown>) =>
    mutate(handle, sessionId, (s) => applyHeader(s, header))
  store.updateContext = (sessionId: string, info: Record<string, unknown>) =>
  ...
  store.startTurn = (sessionId: string, turn: number) =>
    mutate(handle, sessionId, (s) => {
...
/** 有统计的会话列表（按最后活动时间倒序）。 */
function sessionsOf(handle: PersistHandle & { seq: number }): Array<{
  sessionId: string
  requests: number
  alerts: number
  lastTime: number
...
function promptOf(usage: Record<string, unknown> | null | undefined): number {
  if (usage === null || typeof usage !== 'object') return 0
  return numberOr(usage.inputTokens, 0) + numberOr(usage.cacheReadTokens, 0) + numberOr(usage.cacheWriteTokens, 0)
}

/** 累加 usage 到目标桶。 */
function addUsage(target: Record<string, number>, usage: Record<string, unknown>): void {
...
/** 告警自增 id。 */
function nextId(handle: PersistHandle & { seq: number }): number {
  handle.seq += 1
  return handle.seq
}
 ...
 */
function dispose(handle: PersistHandle & { seq: number }): Promise<void> {
  if (!handle.ready) {
    ...
    return handle.readyPromise.then(async () => {
      if (handle.dirty) await handle.persistNow?.()
      await handle.drainWrites?.()
    })
  }
  if (handle.dirty) void handle.persistNow?.()
