...
const SETTINGS_SWITCHES: GuardSettingsSwitchSpec[] = [
...
const SETTINGS_MODE_OPTIONS: GuardSettingsModeOption[] = [
...
function settingsDraftOf(value: unknown): GuardSettingsDraft {
...
function selectedModeOf(mode: string): GuardSettingsModeOption {
  return SETTINGS_MODE_OPTIONS.find((option) => option.id === mode) ?? SETTINGS_MODE_OPTIONS[0]
}

/** 开关行。 */
function settingsSwitchRow(props: GuardSettingsSwitchProps): unknown {
...
/** 选择行（模式三选一）。 */
function settingsSelectRow(props: GuardSettingsSelectProps): unknown {
  return createElement(
    'select',
    {
...
/** 模式区块（下拉 + 当前模式说明）。 */
function settingsModeSection(view: GuardSettingsViewState): unknown {
...
/** 检测开关区块（投毒扫描 / 提示注入检测）。 */
function settingsScanSection(view: GuardSettingsViewState): unknown {
  return createElement(
    'div',
    { className: 'dsh-my-guard-settings-section' },
    ...
    ...SETTINGS_SWITCHES.map((item) =>
      createElement(settingsSwitchRow, {
        ...
        onChange: (v: boolean) => (item.key === 'poisonScan' ? view.setPoisonScan(v) : view.setInjection(v)),
      }),
    ),
  )
}
...
/** 告警通知区块（通知开关 + 冷却秒数）。 */
function settingsNotifySection(view: GuardSettingsViewState): unknown {
  return createElement(
    'div',
    { className: 'dsh-my-guard-settings-section' },
...
function settingsRulesSection(view: GuardSettingsViewState): unknown {
  return createElement(
    'div',
    { className: 'dsh-my-guard-settings-section' },
...
/** 保存区（按钮 + 成功/失败提示）。 */
function settingsActions(view: GuardSettingsViewState): unknown {
  return createElement(
    'div',
...
function guardSettingsView(view: GuardSettingsViewState): unknown {
  return createElement(
    'div',
...
/** 加载中视图。 */
function settingsLoadingView(): unknown {
  return createElement(
    'div',
    { className: 'dsh-my-guard-settings' },
...
/** 加载失败视图（原因 + 重试）。 */
function settingsLoadErrorView(reason: string, retry: () => void): unknown {
  return createElement(
    'div',
    { className: 'dsh-my-guard-settings' },
...
function persistGuardSettings(current: GuardSettingsDraft, setters: GuardSettingsSetters): Promise<void> {
...
function GuardSettingsView(): unknown {
  const [draft, setDraft] = useState<GuardSettingsDraft | null>(null)
  ...
  const patchDraft = (patch: Partial<GuardSettingsDraft>) => setDraft({ ...draft, ...patch })
  return guardSettingsView({
    ...draft,
    busy,
    saved,
    saveError,
    setMode: (mode: string) => patchDraft({ mode }),
    setPoisonScan: (v: boolean) => patchDraft({ poisonScan: v }),
    setInjection: (v: boolean) => patchDraft({ injection: v }),
    setNotifyEnabled: (v: boolean) => patchDraft({ notifyEnabled: v }),
    setNotifyCooldownSec: (v: number) => patchDraft({ notifyCooldownSec: v }),
 ...
 * 声明，实际渲染发生在之后。
 */
function settingsSlotsOf(ctx: ClientContext): SlotsService | undefined {
  if (typeof ctx.get === 'function') return ctx.get<SlotsService>('slots', false)
  return ctx.slots
}
