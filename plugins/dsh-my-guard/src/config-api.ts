...
export function configValue(options: GuardOptions): GuardConfigValue {
  return {
    mode: options.mode,
...
function boolOr(value: unknown, current: boolean, fallback: boolean): boolean {
...
export function mergeConfigPatch(
  ...
  return {
    next: {
      ...
      notifyCooldownMs:
        payload.notifyCooldownMs === undefined ? options.notifyCooldownMs : normalizeCooldown(payload.notifyCooldownMs),
...
export function createConfigSaver(options: GuardOptions): (next: SaveConfigPayload) => Promise<SaveConfigResult> {
  return async (next: SaveConfigPayload) => {
...
export async function dispatchConfigApi(
