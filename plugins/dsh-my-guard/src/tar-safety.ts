...
/** 校验拒绝结论（与成功形态在类型上互斥，便于按 `ok` 收窄）。 */
interface TarRejection {
  ok: false
  reason: string
}

/** 校验结论：`ok:false` 时 `reason` 说明拒绝原因。 */
export type TarSafety = { ok: true } | TarRejection

/** 一个 entry 的有效属性（已合并长名 / pax 覆盖）。 */
interface Entry {
  name: string
  link: string
  size: number
...
/** 单个 entry 的推进结论：通过时可带回「下一个 entry 的属性覆盖」。 */
type StepResult = { ok: true; override?: Override } | TarRejection
...
export async function inspectTarStream(
  ...
  const globalOverrides: Override = {}
  ...
  try {
    for (;;) {
      ...
      if (isZeroBlock(block)) continue // 结束零块与尾部填充
...
async function checkEntry(
  ...
  if (METADATA_TYPES.has(entry.type)) {
    if (entry.size > MAX_META_BYTES) return { ok: false, reason: `tar 元数据 entry 体积异常（${entry.size} 字节）` }
  ...
  if (!(await reader.skip(blocks))) return { ok: false, reason: 'tar 数据块不完整（截断）' }
  return { ok: true }
}

/** 解包体积超限的拒绝原因（累计声明体积 + 上限换算成 MiB，便于用户判断）。 */
function overDeclared(total: number, limit: number): string {
  return `声明解包体积累计 ${total} 字节超过上限 ${Math.floor(limit / (1024 * 1024))} MiB，拒绝解包（防解压炸弹）`
}

/** entry 是否不安全；返回拒绝原因或 `null`。 */
function unsafeReason(entry: Entry): string | null {
  const named = unsafeNameReason(entry.name)
  if (named !== null) return named
  if (entry.type === '2') return unsafeLinkReason(entry, '符号链接')
  if (entry.type === '1') return unsafeLinkReason(entry, '硬链接')
  if (!ALLOWED_TYPES.has(entry.type)) return `不支持的 tar entry 类型 ${JSON.stringify(entry.type)}`
  return null
}

/** 名字层面的通用规则：绝对路径 / 盘符 / 反斜杠分隔符 / `..` 穿越。 */
function unsafeNameReason(name: string): string | null {
  if (name === '') return 'tar entry 名为空'
  if (name.includes('\0')) return 'tar entry 名含 NUL 字节'
  if (name.startsWith('/')) return `tar entry 使用绝对路径（${clip(name)}）`
  if (/^[A-Za-z]:/.test(name)) return `tar entry 使用 Windows 盘符（${clip(name)}）`
  if (name.includes('\\')) return `tar entry 名含反斜杠分隔符（${clip(name)}）`
  if (name.split('/').includes('..')) return `tar entry 路径穿越（${clip(name)}）`
  return null
}

/** 软/硬链接目标必须落在解包目录内（软链按所在目录解析，硬链按归档根解析）。 */
function unsafeLinkReason(entry: Entry, kind: string): string | null {
  const { link } = entry
  const suspicious =
    link === '' ||
    link.startsWith('/') ||
...
/** 链接目标归一化后的相对路径（相对解包根）。 */
function resolvedTarget(entry: Entry, kind: string): string {
  if (kind === '硬链接') return posix.normalize(entry.link)
...
function metadataPatch(entry: Entry, data: Buffer): StepResult {
  const text = data.toString('utf8').replace(/\0+$/, '')
  if (entry.type === 'L') return { ok: true, override: { name: text } }
  if (entry.type === 'K') return { ok: true, override: { link: text } }
  const records = parsePax(text)
  if (!records.ok) return records
  const override: Override = {}
  for (const [key, value] of Object.entries(records.value)) {
    if (key === 'path') override.name = value
    else if (key === 'linkpath') override.link = value
    else if (key === 'size') {
      const size = Number(value)
      if (!Number.isSafeInteger(size) || size < 0) return { ok: false, reason: `PAX size 记录非法（${clip(value)}）` }
      override.size = size
    }
  }
...
function parsePax(text: string): { ok: true; value: Record<string, string> } | TarRejection {
  ...
  while (rest !== '') {
    ...
    if (eq <= 0) return { ok: false, reason: `PAX 记录缺少 key（${clip(record)}）` }
...
/** 合并覆盖：只覆盖被显式声明的字段。 */
function applyOverride(entry: Entry, overrides: Override): Entry {
  return {
...
function parseHeader(block: Buffer): { ok: true; entry: Entry } | TarRejection {
  if (!checksumOk(block)) return { ok: false, reason: 'tar header 校验和不符（结构非法）' }
  const size = parseNumber(block.subarray(124, 136))
  if (size === null) return { ok: false, reason: 'tar header size 字段非法' }
  const name = joinPrefix(cstr(block, 0, 100), cstr(block, 345, 155))
  const type = (block[156] === 0 ? '0' : String.fromCharCode(block[156])) as string
  return { ok: true, entry: { name, link: cstr(block, 157, 100), size, type } }
}

/** ustar `prefix` 与 `name` 拼接（GNU tar 同样如此），否则逃逸路径可藏在 prefix 里。 */
function joinPrefix(name: string, prefix: string): string {
...
/** 数据块数（含填充）。 */
function blocksOf(size: number): number {
