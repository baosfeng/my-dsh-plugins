...
/** tarball 获取失败类别。 */
export type TarballFailure = 'network' | 'timeout' | 'http-status' | 'bad-metadata' | 'integrity' | 'io-error'

/** 失败结论：带类别 + 可诊断信息（状态码 / URL / 缺失字段）。 */
export interface TarballFetchFailure {
  ok: false
  kind: TarballFailure
  error: string
}

/** tarball 获取结果：成功带落盘路径。 */
export type TarballFetch = { ok: true; file: string } | TarballFetchFailure

/** 获取参数（registry 基址与超时可注入，便于回归测试）。 */
...
export function classifyFetchFailure(error: unknown): TarballFailure {
  const name = (error as { name?: unknown } | null | undefined)?.name
  if (name === 'TimeoutError' || name === 'AbortError') return 'timeout'
  const causeCode = (error as { cause?: { code?: unknown } } | null | undefined)?.cause?.code
  if (typeof causeCode === 'string' && causeCode !== '') return 'network'
  if (error instanceof TypeError) return 'network' // undici 把网络失败包成 TypeError: fetch failed
  return 'io-error'
}

/** registry 元数据里后续要用的两个字段。 */
 ...
 */
export async function fetchTarball(pkg: string, options: TarballFetchOptions = {}): Promise<TarballFetch> {
...
async function loadMeta(
  metaUrl: string,
  timeoutMs: number,
): Promise<({ ok: true } & RegistryMeta) | TarballFetchFailure> {
...
function readDist(meta: Record<string, unknown>, metaUrl: string): ({ ok: true } & RegistryMeta) | TarballFetchFailure {
...
async function downloadTarball(
  url: string,
  timeoutMs: number,
): Promise<{ ok: true; body: Buffer } | TarballFetchFailure> {
  try {
...
/** 落盘到临时文件（摘要校验通过后才写）。 */
async function persistTarball(body: Buffer): Promise<TarballFetch> {
  try {
...
function fetchFailure(error: unknown, what: string, url: string, timeoutMs: number): TarballFetchFailure {
