...
function caretUpper(lo) {
    if (lo.major > 0)
        return { major: lo.major + 1, minor: 0, patch: 0, prerelease: ['0'] };
    if (lo.minor > 0)
        return { major: 0, minor: lo.minor + 1, patch: 0, prerelease: ['0'] };
    if (lo.patch > 0)
        return { major: 0, minor: 0, patch: lo.patch + 1, prerelease: ['0'] };
    return { major: 0, minor: 1, patch: 0, prerelease: ['0'] };
}
function satisfiesCaret(version, spec) {
    const base = parseRangeVersion(spec);
    const parsed = parseVersion(version);
    if (base === null || parsed === null)
        return false;
    const lo = lowerBound(base);
    if (compareParsed(parsed, lo) < 0)
        return false;
...
function satisfiesToken(version, token) {
    if (token === '' || token === '*' || token === 'x' || token === 'X')
        return true;
    if (token.startsWith('^'))
        return satisfiesCaret(version, token.slice(1));
    if (token.startsWith('~'))
        return satisfiesTilde(version, token.slice(1));
    if (/^(>=|<=|>|<|=|==)/.test(token))
...
export function satisfies(version, range) {
    ...
    if (trimmed === '' || trimmed === '*')
        return true;
    const parsed = parseVersion(version);
    if (parsed !== null && parsed.prerelease.length > 0 && !range.includes('-'))
        return false;
