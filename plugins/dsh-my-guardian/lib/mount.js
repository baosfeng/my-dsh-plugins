...
export function findRootTree(loader) {
    try {
        for (const entry of loader.entries()) {
            ...
            if (tree === undefined || typeof tree.filename !== 'string')
                continue;
...
async function mount(shared, id, options) {
    ...
    if (options.config !== undefined && options.config !== null)
        entryOptions.config = options.config;
...
async function promote(shared, recordKey, id, name, entry, record) {
    if (recordKey === 'staged') {
        ...
        if (next.length !== entries.length)
            await writeStagedFile(shared.stagedFile, next);
    }
    else {
        shared.state.promoted[id] = {
            ...record,
...
async function processStagedEntry(shared, id, entry) {
    if (typeof id !== 'string' || id === '' || entry === null || typeof entry !== 'object')
        return;
    const e = entry;
    if (typeof e.name !== 'string' || e.name === '')
        return;
    if (shared.attempted.has(id))
        return;
...
export async function initialScan(shared) {
    ...
    if (!Array.isArray(shared.state.events))
        shared.state.events = [];
    if (shared.state.staged === null || typeof shared.state.staged !== 'object')
        shared.state.staged = {};
    if (shared.state.promoted === null || typeof shared.state.promoted !== 'object')
        shared.state.promoted = {};
    if (typeof shared.state.safeMode !== 'boolean')
