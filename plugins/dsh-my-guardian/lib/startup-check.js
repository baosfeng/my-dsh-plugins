 ...
 * own apply:
 *
 *   - package resolvability: every dsh-* roster entry's package must exist
 *     under the profile node_modules — an unresolvable package always fails
 *     the import stage (assertEntriesLoaded throws in dsh-app-boot)
 *   - peer dependencies: reuse the staged-mount pre-check (dep-precheck.js)
 *     — missing/out-of-range peers surface at boot as module-resolution
 *     failures, so they are flagged before the next restart hits them
 *   - duplicate entry ids: EntryGroup.update in cordis-plugin-loader throws
 *     "duplicate loader entry id" when a group config lists the same id
 *     twice — a guaranteed boot failure worth flagging up front
 *
 * The report is written to $DSH_HOME/guardian/startup-issues.json so it
 * survives a broken boot: applies run in parallel with roster imports, so the
 * guardian can usually record the report before assertEntriesActivated
 * rejects the boot.
 *
...
function profileNodeModules(profileDir) {
...
export function collectTreeEntries(tree) {
  ...
  return Object.values(store).filter((entry) => entry !== null && typeof entry === 'object')
}
...
export function isDisabledEntry(entry) {
  ...
  return options !== null && typeof options === 'object' && options.disabled === true
}
...
function duplicateIssue(id, names) {
  return {
    type: 'duplicate-id',
    entryId: id,
    name: names[0] ?? id,
...
function checkPluginItem(issues, item, nmRoot, profileDir) {
 ...
 */
export async function runStartupCheck(ctx, shared) {
  ...
  await writeStartupIssuesFile({ version: 1, checkedAt, profileDir: shared.profileDir, issues })
