...
interface SnapshotResult {
  safeMode: boolean
  staged: EntrySnapshot[]
  promoted: EntrySnapshot[]
...
export function createApi(ctx: DshContext, shared: SharedContext): Pick<SharedContext, 'ensureApi' | 'snapshot'> {
  return {
 ...
 */
function ensureApi(ctx: DshContext, shared: SharedContext): void {
...
async function handleApiRequest(
  ...
  webRuntime: WebRuntimeService | undefined,
...
async function dispatchApiMethod(
  ctx: DshContext,
  shared: SharedContext,
  method: string,
  request: ServerRequest,
...
async function handleStagedPost(
  _ctx: DshContext,
  shared: SharedContext,
  request: ServerRequest,
  response: ServerResponse,
  ...
  if (entries.some((item) => (item as StagedEntry)?.id === id)) {
...
async function handleSafemodePost(
  _ctx: DshContext,
  shared: SharedContext,
  request: ServerRequest,
  response: ServerResponse,
...
function entrySnapshot(shared: SharedContext, id: string, record: EntryRecord, isStaged: boolean): EntrySnapshot {
...
function snapshot(shared: SharedContext): SnapshotResult {
