 ...
 */
export function captureTeardownEntries(store: Record<string, unknown> | null | undefined): Set<string> {
  if (store === null || store === undefined || typeof store !== 'object') return new Set()
 ...
 */
export function diffReleasedEntries(
  ...
  for (const id of before) {
    if (!after.has(id)) released.push(id)
  }
  return released
}
 ...
 */
export function recordTeardownReleases(shared: SharedContext, before: ReadonlySet<string>): string[] {
