...
- **工具查询**：`memory_query` 工具（只读）让 agent 查询记忆详情——全局/项目过滤 + 关键词过滤，项目 cwd 取会话工作目录；**输出与渲染文本都带每条记忆的分类标签**（偏好/事实/项目/技术栈/工作流，issue #192），空结果文案保持不变。
- **工具保存**（issue #107 / #192）：`memory_save` 工具让 agent 主动保存记忆（`scope`/`desc` 必填、`category` 可选——偏好 `preference`/事实 `fact`/项目 `project`/技术栈 `stack`/工作流 `workflow`，默认 `fact`；`cwd` 可选）——每次调用都经 **DSH 原生审批确认**（`tools/pre-execute` 返回 `{ kind: 'ask' }`，用户确认后才写入），记忆绝不静默变更；保存后 `memory_query` 立即可查、后续会话注入生效。
- **工具删除**（issue #192）：`memory_delete` 工具让 agent 按 id 删除一条记忆（`id` 必填，`scope`/`cwd` 与保存同语义，默认全局）——与 `memory_save` **共用同一道 `tools/pre-execute` 确认门与同一套 `saveApproval` 权限模式策略**（#208），确认文案带上待删内容摘要（「agent 请求删除全局记忆「用 pnpm 装依赖」[mem-…]。删除不可撤销，请确认是否删除」），未确认/被拒绝时条目不删；删除不存在的 id **明确报错**（不静默成功）；删除后 `memory_query` 立即查不到，日志留下 itemId + 调用会话的审计线索，输出把被删条目原样回执给模型。
...
5. 新会话开始时，全局记忆自动注入系统提示词（agent 可见）；agent 可用 `memory_query` 工具查询记忆详情（含项目记忆），也可用 `memory_save` 工具保存记忆——**默认仍需用户确认**；仅在会话权限模式为「完全权限」（`danger-full-access`，即 approval policy = `never`）时按 `saveApproval` 策略免确认直接写入（条目带来源会话标记、面板可见可删除，issue #208）。发现值得记住的信息时可请 agent 保存（如「把「用 pnpm 装依赖」记到记忆里」，也可指定类型：「这条记成偏好」）；要删记忆时让 agent 用 `memory_delete` 按 id 删（同样先弹确认，确认后才真正删除；`memory_query` 的输出里带 id 与分类，issue #192）。
...
- **删除同策略**（issue #192）：`memory_delete` 复用上表同一矩阵——`auto` + `never` 免确认删除（日志留痕）、`always` + `never` 明确失败并给出可操作提示、`never` 从不确认；`ask` 模式（如 workspace-write）下删除同样弹原生确认，确认文案包含待删内容摘要，用户拒绝即不删除。
...
// cordis.yml 中 my-memory 的 config 字段
{
  ...
  "saveApproval": "auto", // issue #208/#192 记忆写操作（保存 + 删除）确认策略：auto（默认）| always | never —— 见下方「权限模式与保存确认」
}
```
...
- **工具**：`lib/tool.js`（`memory_query` 只读 + `memory_save` 写，工具描述引导「浓缩为 1-2 句」）、`lib/delete-tool.js`（`memory_delete`，issue #192）、`lib/write-tools.js`（两个写工具的注册 + 确认门装配，issue #192 拆出以守住 400 行文件门禁）直接构造 ToolDefinition（JSON Schema 参数/输出，不导入 `@deepseek-ai/dsh-tools`——本仓库插件只解析 Node 内置模块与相对路径）；`category` 枚举与中文标签由 `lib/memory-scoring.js` 的 `CATEGORIES`/`categoryLabelOf` 单一来源提供，输出 schema 复用 #191 的 `MEMORY_ITEM_SCHEMA`（save/query/delete 同一对象引用，形状不漂移）。
- **保存/删除确认门**（issue #107 / #192 / #208）：`lib/save-policy.js` 的 `createMemorySaveGate` 注册在 `tools/pre-execute`，**同一道门管辖 `memory_save` 与 `memory_delete`**，按 `saveApproval` × 会话 approval policy 决策（allow / ask / deny + 可操作提示），ask/deny 文案按「保存 / 删除」区分；删除的 ask 文案经 `lookupTarget`（index 侧接到真实 store 的 `lookupDeleteTarget`）解析待删内容摘要，解析不出时仍照常询问（绝不静默放行）；`approvalPolicyOf` 优先调用宿主 `ApprovalService.effectivePolicy(session)`，回落会话日志 `approval/policy` 折返（`exec.agent.session` 的 `seq` + `eventAt`）；写入统一带 `source: { sessionId, at }`（`exec.agent.id` 即会话 id，宿主 `dsh-agent` 校验 `agent.id === agent.session.id`）。
