...
export function createMemoryDeleteTool({ globalStore, getProjectStore, logger }) {
    return {
        name: 'memory_delete',
        description: '删除记忆（写操作，不可撤销，需用户确认）：按 id 删除全局或项目记忆中的一条。id 取自 memory_query 的输出，删除前建议先 memory_query 确认要删的是哪一条。删除不存在的 id 会明确报错，不会静默成功。',
...
async function executeDelete(args, exec, { globalStore, getProjectStore, logger }) {
    ...
    if (item === undefined) {
        warnDelete(logger, `memory_delete 未找到条目（scope=${target.scope}，itemId=${id}，sessionId=${sessionId}，操作=delete）`);
...
export async function lookupDeleteTarget(args, deps, exec) {
...
async function locateTarget(args, { globalStore, getProjectStore }, exec) {
...
export function renderDeleteResult(value) {
