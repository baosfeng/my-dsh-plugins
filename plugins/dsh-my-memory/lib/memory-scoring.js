/** 记忆分类（issue #78 结构化索引）。 */
export const CATEGORIES = ['preference', 'fact', 'project', 'stack', 'workflow'];
/** 默认分类（旧数据/未标注候选回退）。 */
...
export function mergeCandidate(items, candidate, now = Date.now()) {
    ...
    const index = list.findIndex((item) => sameTheme(withDefaults(item, now), candidateWithDefaults) ||
    ...
    if (!sameText)
        merged.status = 'conflict-pending';
    merged.history = [
...
export function decayConfidence(items, now = Date.now(), thresholdMs = DEFAULT_DECAY_MS) {
    ...
    return {
        items: list.map((item) => {
            ...
            if (decayed === normalized.confidence)
                return normalized;
...
export function scoreForInjection(item, context, opts) {
    ...
    return {
        id: normalized.id,
        item: normalized,
        score: options.weights.relevance * relevance +
...
export function relevanceOf(desc, keywords) {
    ...
    if (keywords.length === 0)
        return 0;
    const hits = keywords.filter((keyword) => typeof keyword === 'string' && keyword !== '' && text.includes(keyword.toLowerCase()));
...
export function pickForInjection(items, context, opts) {
    ...
    const maxConfidence = Math.max(1, ...wellFormed.map((item) => (Number.isFinite(item?.confidence) ? item.confidence : 1)));
