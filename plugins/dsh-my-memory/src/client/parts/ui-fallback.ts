...
function uiIconFallback(glyph: (size: number) => ReactNode, size: number, name: string) {
  return function UiIconFallback(props?: { className?: string }): ReactNode {
...
function UiPillFallback(props?: Record<string, unknown>): ReactNode {
...
function UiButtonFallback(props?: Record<string, unknown>): ReactNode {
...
function UiInputFallback(props?: Record<string, unknown>): ReactNode {
  const { icon, className, ...rest } = props ?? {}
  const wrapperProps: Record<string, unknown> = { style: { display: 'inline-flex', alignItems: 'center' } }
  if (typeof className === 'string') wrapperProps.className = className
