...
function askReasonOf(args: { scope?: string; desc?: string } | undefined): string {
...
export function decideSaveGate(input: {
  args?: { scope?: string; desc?: string }
  policy: ApprovalPolicy | undefined
  config?: SavePolicyConfig
}): SaveDecision {
  const mode = saveApprovalOf(input.config)
  if (mode === 'never') return { kind: 'allow' }
  if (mode === 'always' && input.policy === 'never') return { kind: 'deny', reason: ALWAYS_UNDER_NEVER_HINT }
...
export function createMemorySaveGate(options?: SaveGateOptions) {
  return async (
    exec: { name?: string; arguments?: { scope?: string; desc?: string }; agent?: unknown },
    next: () => Promise<unknown>,
  ) => {
    const downstream = await next()
    if (exec?.name !== 'memory_save') return downstream
    return saveGateDecision(exec, downstream, options)
  }
}

/** 决策 + 日志（拆出为独立函数以守住圈复杂度 ≤ 10 门禁）。 */
function saveGateDecision(
  exec: { arguments?: { scope?: string; desc?: string } },
  downstream: unknown,
