...
function apiHandler(fence, options, bus, onConfigChange, emitNotice, webhookStore) {
    return async (request, response) => {
        ...
        try {
            const handled = await dispatchMethod(method, request, response, options, bus, onConfigChange, emitNotice, webhookStore);
            if (!handled) {
...
function normalizeAskMode(payload) {
    if (payload.askMode === undefined)
        return {};
    if (payload.askMode !== 'full' && payload.askMode !== 'summary')
        return undefined;
...
function normalizeDedupeMs(payload) {
    if (payload.dedupeMs === undefined)
        return {};
    if (!Number.isFinite(payload.dedupeMs))
        return undefined;
