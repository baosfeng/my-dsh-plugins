...
/** agent 状态事件的搜索片段。 */
function agentStatusParts(data: Record<string, unknown>): unknown[] {
  return [data.status, data.agentType].filter((part) => typeof part === 'string')
}

/** 模型流事件的搜索片段（含错误消息）。 */
function llmParts(data: Record<string, unknown>): unknown[] {
...
function toolCallParts(data: Record<string, unknown>): unknown[] {
  const parts: unknown[] = []
  if (typeof data.name === 'string') parts.push(data.name)
  if (Array.isArray((data.args as any)?.keys)) parts.push(...(data.args as any).keys)
  if (typeof (data.args as any)?.summary === 'string' && (data.args as any).summary !== '')
    parts.push((data.args as any).summary)
  return parts
}

/** 工具结果事件的搜索片段（工具名 + 成败）。 */
function toolResultParts(data: Record<string, unknown>): unknown[] {
  const parts: unknown[] = []
  if (typeof data.name === 'string') parts.push(data.name)
...
function pluginEventParts(data: Record<string, unknown>): unknown[] {
  const parts: unknown[] = []
  ...
  if (data.params !== null && typeof data.params === 'object') {
    for (const value of Object.values(data.params as Record<string, unknown>)) {
...
/** 事件类型 → 搜索片段收集函数（查表消分支）。 */
const PARTS_COLLECTORS: Record<string, (data: Record<string, unknown>) => unknown[]> = {
...
export function searchableText(event: unknown): string {
  const data = (event as any) && (event as any).data ? (event as any).data : {}
  const parts: unknown[] = [(event as any)?.type, (event as any)?.sessionId]
  const collector = PARTS_COLLECTORS[(event as any)?.type]
...
export function matchesKeyword(event: unknown, keyword: unknown): boolean {
...
function isFailEvent(event: unknown): boolean {
  if ((event as any)?.type === 'tool_result') return (event as any).data?.ok === false
  if ((event as any)?.type === 'llm_stream') return (event as any).data?.phase === 'error'
  return false
}

/** 归一化过滤条件（时间转为闭区间数值；空值透传）。 */
function normalizeCriteria(criteria: AuditCriteria): {
  type: string
  keyword: string
  result: string
...
function passType(type: unknown, filterType: string): boolean {
...
/** 时间范围闭区间。 */
function passTime(time: number, start: number | undefined, end: number | undefined): boolean {
...
function passResult(event: unknown, result: string): boolean {
...
export function applyAuditFilter(events: unknown, criteria: AuditCriteria = {}): unknown[] {
  const ctx = normalizeCriteria(criteria)
  return ((events ?? []) as AuditEventLike[]).filter(
    (event) =>
...
export function toolNameOf(event: unknown): string {
  if ((event as any)?.type === 'tool_call' || (event as any)?.type === 'tool_result')
    return String((event as any).data?.name ?? '')
  return ''
}

/** 事件的结果摘要（成功/失败/错误；其余空）。 */
export function resultTextOf(event: unknown, labels: CsvLabels = DEFAULT_CSV_LABELS): string {
  if ((event as any)?.type === 'tool_result')
    return (event as any).data?.ok === false ? (labels.fail as string) : (labels.ok as string)
  if ((event as any)?.type === 'llm_stream' && (event as any).data?.phase === 'error') return labels.error as string
  return ''
}

/** 生成 CSV 摘要（表头：时间/类型/工具/结果）。labels 可覆盖默认中文。
 *  返回不含换行结尾符的 CSV 文本。 */
export function auditToCsv(events: unknown, labels: CsvLabels = DEFAULT_CSV_LABELS): string {
  const typeMap = labels.typeMap ?? {}
  const header = [labels.time, labels.type, labels.tool, labels.result]
  const lines = [header.map(csvCell).join(',')]
  for (const event of (events ?? []) as AuditEventLike[]) {
    const typeLabel = typeMap[event.type as string] ?? String(event.type)
...
/** 生成 JSON 完整数据（缩进默认 2）。 */
export function auditToJson(events: unknown, space = 2): string {
  return JSON.stringify(events ?? [], null, space)
}

/** 事件是否为工具类（tool_call / tool_result）。 */
function isToolEvent(event: AuditEventLike): boolean {
...
function bumpTool(byTool: Map<string, ToolEntry>, event: AuditEventLike, name: string): void {
...
/** 按工具名聚合调用次数与失败次数。 */
function aggregateToolStats(events: unknown): Map<string, ToolEntry> {
...
/** 聚合结果 → 排序 + 失败率列表。 */
function rankTools(byTool: Map<string, ToolEntry>): ToolStat[] {
...
export function computeToolStats(events: unknown, topN = 5): ToolStat[] {
 ...
 *  空关键词返回整段未命中。 */
export function highlightSegments(text: unknown, keyword: unknown): HighlightSegment[] {
