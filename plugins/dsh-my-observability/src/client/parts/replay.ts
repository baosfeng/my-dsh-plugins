...
/** 事件类型 → 中文标签。 */
function typeLabel(event: any): string {
  switch (event.type) {
    case 'agent_status':
...
function typeKind(event: any): string {
...
function typeIcon(event: any): unknown {
...
/** 时间戳 → HH:MM:SS。 */
function timeText(time: any): string {
  try {
...
/** agent 类型标记 → 中文。 */
function agentTypeText(agentType: any): string {
...
/** 模型流阶段 → 中文。 */
function phaseText(phase: any): string {
...
/** agent 状态事件摘要。 */
function agentMeta(data: any): string {
  return `状态 ${data.status} · ${agentTypeText(data.agentType)}`
}

/** 模型流事件摘要（开始/结束/错误 + 统计）。 */
function llmMeta(data: any): string {
...
function eventDetail(event: any): unknown {
...
function EventRow({ event, keyword }: { event: any; keyword?: string }): unknown {
...
function TypeFilter({ filter, onFilter }: { filter: string; onFilter: (value: string) => void }): unknown {
  const options = [
    ['', strings.filterAll()],
...
function sessionOptionLabel(s: any): string {
...
function shortId(sessionId: any): string {
...
/** 错误状态（错误色文案 + 重试按钮）。 */
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }): unknown {
  return createElement(
    'div',
 ...
 *  面板可见时轮询、隐藏时暂停。 */
function ReplayPanel(props: any): unknown {
