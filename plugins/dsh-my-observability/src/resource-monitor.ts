...
import type { ResourceAlert, ResourceLimits, ResourceSample } from './resource-rules.js'
...
export interface ResourceSnapshot extends ResourceSample {
...
function sample(
  ...
  if (prev !== null) {
    ...
    const sample: ResourceSample = { time: now, cpuPercent, memoryBytes, fileBytes, writeRateBytesPerHour, homeBytes }
...
/** 启动周期采样（幂等）。 */
function startMonitor(state: MonitorState, intervalMs: number, monitor: ResourceMonitor): NodeJS.Timeout | null {
  if (state.timer === null) {
...
/** 停止采样（幂等）。 */
function stopMonitor(state: MonitorState): void {
  if (state.timer !== null) {
