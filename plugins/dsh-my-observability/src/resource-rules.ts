 ...
 */
function isCriticalOverLimit(sample: ResourceSample, limits: ResourceLimits = DEFAULT_LIMITS): boolean {
  return (
...
export function shouldEnterDegrade(
  ...
  limits: ResourceLimits = DEFAULT_LIMITS,
  confirmCount = DEGRADE_CONFIRM_COUNT,
): boolean {
...
export function shouldExitDegrade(
  history: ResourceSample[],
  limits: ResourceLimits = DEFAULT_LIMITS,
  confirmCount = RECOVER_CONFIRM_COUNT,
): boolean {
  const recent = history.slice(-confirmCount)
  if (recent.length < confirmCount) return false
  return recent.every((sample) => !isCriticalOverLimit(sample, limits))
