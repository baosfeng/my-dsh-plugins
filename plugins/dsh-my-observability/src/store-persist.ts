...
/** 追加日志文件路径（新格式）。 */
export function jsonlFile(): string {
  return join(dataHome(), 'observability', 'audit.jsonl')
}

/** 兼容的旧格式文件路径（0.1.x 整文件 JSON）。 */
export function legacyFile(): string {
  return join(dataHome(), 'observability', 'audit.json')
}

/** 事件结构校验（时间/会话/类型为合理形态）。 */
function isValidEvent(event: unknown): event is AuditEvent {
  const target = event as AuditEvent | null | undefined
  return (
    target !== null &&
 ...
 */
export function normalizeLoaded(bySession: Record<string, unknown>): AuditState {
  ...
  for (const [sessionId, bucket] of Object.entries(bySession)) {
    const raw = Array.isArray(bucket)
      ? bucket
      : Array.isArray((bucket as SessionBucket | null | undefined)?.events)
        ? ((bucket as SessionBucket).events as unknown[])
        : []
...
export function parseJsonl(text: string): { bySession: Record<string, AuditEvent[]>; lines: number } {
  const bySession: Record<string, AuditEvent[]> = {}
  let lines = 0
...
export function parseLegacy(text: string): { bySession: Record<string, unknown>; lines: number } | null {
  try {
    const parsed: unknown = JSON.parse(text)
    if (parsed === null || typeof parsed !== 'object') return null
    const bySession = (parsed as { bySession?: unknown }).bySession
    if (typeof bySession !== 'object' || bySession === null) return null
    return { bySession: bySession as Record<string, unknown>, lines: Object.keys(bySession).length }
  } catch {
    return null
  }
}
 ...
 */
export async function loadPersisted(file: string, legacy: string): Promise<LoadResult> {
...
function stateToLines(state: AuditState): string {
  const lines: string[] = []
...
/** 追加写入（jsonl 行）；目录自动创建。 */
export async function appendLines(file: string, text: string, logger?: PersistLogger, prefix?: string): Promise<void> {
  try {
...
export async function removeLegacyAfterMigration(legacy: string): Promise<void> {
  try {
