...
function pickVersion(packument, version, latest) {
    ...
    if (requested !== '' && isPlainObject(packument.versions?.[requested]))
        return requested;
    return latest;
}
...
function runtimeProvidedModule(name) {
    if (name === 'react' || name === 'react-dom' || name === 'cordis' || name.startsWith('cordis:'))
        return true;
...
function cleanRepositoryUrl(raw) {
    ...
    if (url.startsWith('github:'))
        ...
        url = `https://${url.slice(10).replace(':', '/')}`;
...
function entryToResult(pkg) {
    ...
    for (const field of STRING_FIELDS)
        result[field] = stringOf(pkg?.[field]);
