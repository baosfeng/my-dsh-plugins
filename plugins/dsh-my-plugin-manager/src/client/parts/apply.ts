...
/** client 端 Context 的最小契约（effect + 动态服务获取）。 */
interface ClientContext {
  effect(callback: () => void | (() => void), label?: string): void
  /** ctx.get(name, strict)：strict = false 时按「服务是否已提供」判断。 */
  get(name: string, strict?: boolean): SlotsService | undefined
}

/** 官方 slots 服务（设置页扩展点）。 */
interface SlotsService {
  /** 注册扩展点实现（返回 disposer）。 */
  inject(name: string, register: () => unknown): void | (() => void)
