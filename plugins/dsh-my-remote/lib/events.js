...
export function attachEvents(ctx, shared) {
    ...
    if (shared.options.end)
        disposers.push(attachEndListener(ctx, shared));
    if (shared.options.ask)
        disposers.push(attachAskInterceptor(ctx, shared));
    if (shared.options.approval)
        disposers.push(attachApprovalInterceptor(ctx, shared));
    return disposers;
}
// ── end 监听 ──────────────────────────────────────────────────────────────
...
function attachEndListener(ctx, shared) {
    return ctx.on('agent/status', ({ agent, status }) => {
        if (status !== 'idle' || agent === undefined || agent === null)
            return;
...
function attachAskInterceptor(ctx, shared) {
    return ctx.on('tools/execute', async (exec, next) => {
        ...
        if (result === entry)
            return remoteAskResult(shared, sessionId, entry);
        if (result === ASK_TIMEOUT) {
            shared.askRegistry.cleanSession(sessionId);
            shared.logger?.warn(`[dsh-my-remote] ask 等待远程回答超时（sessionId=${sessionId}，超时=${shared.options.askTimeoutMs}ms，返回空回答由模型自行决策）`);
...
function askTarget(exec, shared) {
    ...
    if (agent === undefined || agent === null || !shared.isTopLevelAgent(agent))
        return null;
    return agent;
}
...
function attachApprovalInterceptor(ctx, shared) {
    return ctx.on('approval/request', async (req, next) => {
        ...
        shared.logger?.info(`[dsh-my-remote] approval 事件已下行（sessionId=${sessionId}，tool=${typeof req.toolName === 'string' ? req.toolName : ''}）`);
...
function approvalRaceResult(shared, sessionId, entry, result) {
    ...
    if (result === APPROVAL_TIMEOUT) {
        shared.logger?.warn(`[dsh-my-remote] approval 等待远程批准超时（sessionId=${sessionId}，超时=${shared.options.approvalTimeoutMs}ms，fail-closed 拒绝）`);
        return 'rejected';
    }
