...
function webServerOf(source) {
    return source.webServer;
}
/** 挂载两条路由（既有 /remote/api + 新 /remote/api/settings）；已注册过先撤。 */
function mountRoutes(hostCtx, scope, shared, port) {
    for (const dispose of routeDisposers.get(hostCtx) ?? [])
        dispose();
    scope.effect(() => {
        ...
        return () => {
            if (routeDisposers.get(hostCtx) === registered)
...
export function registerRemoteRoutes(ctx, shared, port) {
    ...
    try {
        hostCtx.inject(['webServer'], (scope) => mountRoutes(hostCtx, scope, shared, port));
    }
    catch (error) {
