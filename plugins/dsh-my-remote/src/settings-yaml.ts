...
/** 该行 config 字典（含未暴露给设置页的键，写回时原样保留）。 */
...
export function parseRowConfig(text: string, rowId: string): ConfigDict {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (scalar !== undefined) config[scalar[0]] = scalar[1]
  }
  return config
}

/** 读取该行已有配置（文件不存在 → 空字典，首次保存按空配置合并）。 */
export async function readRowConfig(file: string, rowId: string): Promise<ConfigDict> {
...
function configBlockLines(text: string, rowId: string): string[] {
  ...
  for (let i = start + 1; i < lines.length; i += 1) {
    ...
    if (out.length === 0 && line !== '  config:') continue
    out.push(line)
  }
  out.shift()
  return out
...
function scalarEntry(line: string): [string, unknown] | undefined {
...
function parseScalarValue(raw: string): unknown {
  ...
  if (value in literals) return literals[value]
...
function parseWebhookLines(lines: string[]): ConfigDict[] {
  const state: WebhookParseState = { entries: [] }
  for (const line of lines) {
    const item = line.match(RUN_HEAD)
    if (item !== null) {
      // `- name:` / `- url:` 是新条目（**必须带 `- `**，否则 8 空格的 `headers:`
      // 会被误判成新条目，headers 整段丢失）。
      if (item[1] === 'name' || item[1] === 'url') state.entry = newWebhookEntry(state)
...
/** webhook 解析状态（跨行累积当前条目与 headers）。 */
interface WebhookParseState {
  entries: ConfigDict[]
  entry?: ConfigDict
...
function newWebhookEntry(state: WebhookParseState): ConfigDict {
  const entry: ConfigDict = {}
...
function assignFieldLine(state: WebhookParseState, line: string): void {
...
function assignInto(state: WebhookParseState, key: string, raw: string): void {
...
export function serializeConfigBlock(config: ConfigDict): string {
  const lines: string[] = []
  for (const [key, value] of Object.entries(config)) {
    if (value === undefined) continue
    if (key === 'webhooks' && Array.isArray(value)) {
      lines.push('    webhooks:')
      lines.push(...serializeWebhooks(value as ConfigDict[]))
      continue
    }
...
function serializeWebhooks(webhooks: ConfigDict[]): string[] {
  const lines: string[] = []
  for (const webhook of webhooks) {
    let first = true
    for (const [key, value] of Object.entries(webhook)) {
      if (key === 'headers' || value === undefined) continue
...
function serializeHeaders(headers: unknown): string[] {
  if (headers === null || typeof headers !== 'object' || Array.isArray(headers)) return []
  const entries = Object.entries(headers as Record<string, unknown>).filter(([, value]) => typeof value === 'string')
 ...
 * 嵌套 webhook 对象（见文件头）。
 */
export async function writeRowEntry(file: string, text: string, rowId: string, entry: string): Promise<void> {
  const kept: string[] = []
  const lines = text.split('\n')
  for (let i = 0; i < lines.length; i += 1) {
    if (lines[i] !== `- id: ${rowId}`) {
    ...
    i += 1
    while (i < lines.length && !TOP_LEVEL.test(lines[i]!) && lines[i] !== '') i += 1
  }
