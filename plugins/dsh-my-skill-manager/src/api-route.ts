...
import type { ScanIssue, ScannedSkill, SkillRoot } from './diagnose.js'
import { usageSnapshot } from './usage.js'
import type { UsageSnapshot, UsageStore } from './usage.js'
import type { DshContext, ServerRequest, ServerResponse, SkillCatalogEntry } from './types.js'

/** createApiHandler 的依赖（由 index.ts 注入）。 */
export interface ApiHandlerDeps {
  ctx: DshContext
  /** 配置变化时让官方 skill 目录立即重算。 */
  invalidate: () => void
  /** 信任围栏（loopback / trustedHosts / 同源）。 */
  fence: (request: ServerRequest) => boolean
  /** 使用统计 store（缺省则不附带 usage 数据）。 */
  usage?: UsageStore
}

/** 列表 payload 中的一行（官方 catalog + 目录扫描合并结果）。 */
...
interface ListPayload {
  ...
  projectRoot: string
  skills: ApiSkillRow[]
  global: { disabled: string[] }
  /** 项目禁用名单（client 侧按数组解析）。 */
  project: string[]
  diagnostics: { missing: ScanIssue[] }
  usage: UsageSnapshot
}
...
async function handleSession(ctx: DshContext, url: URL, response: ServerResponse): Promise<void> {
...
function cwdOf(url: URL): string | undefined {
...
function mergeSkills(
  ...
  const byName = new Map<string, ApiSkillRow>()
...
function disabledNames(global: ConfigDoc, project: ConfigDoc | undefined): Set<string> {
  return new Set([...global.global.disabled, ...projectDisabled(project)])
}

/** 项目配置的禁用名单（无项目配置时为空）。 */
function projectDisabled(project: ConfigDoc | undefined): string[] {
  return project?.project.disabled ?? []
}

/** 按视图过滤 catalog：项目视图只保留项目来源条目。 */
function viewSkills(catalog: SkillCatalogEntry[], safeCwd: string | undefined): SkillCatalogEntry[] {
  return safeCwd === undefined ? catalog : catalog.filter((skill) => isProjectSource(skill.source))
}

/** 按视图过滤扫描根目录：项目视图只保留项目来源 roots。 */
function viewRoots(roots: SkillRoot[], safeCwd: string | undefined): SkillRoot[] {
...
async function handleConfig(request: ServerRequest, response: ServerResponse, invalidate: () => void): Promise<void> {
  const payload = await readJsonBody(request)
  const scope = payload.scope
  const rawDisabled = payload.disabled
  const disabled = Array.isArray(rawDisabled)
    ? rawDisabled.filter((name): name is string => typeof name === 'string' && name !== '')
    : []
