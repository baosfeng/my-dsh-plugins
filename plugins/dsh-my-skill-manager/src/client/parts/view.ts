...
function isProjectSource(source: unknown): boolean {
...
function flipDisabled(list: string[], name: string, isDisabled: boolean): string[] {
  return isDisabled ? list.filter((n) => n !== name) : [...new Set([...list, name])]
}

/** createActions 的 state setter 依赖。 */
interface ActionsDeps {
  setData: (value: ListValue) => void
  setLoading: (value: boolean) => void
  setError: (value: boolean) => void
  setSaved: (value: boolean) => void
}

/** 数据动作（load / rescan / toggle）。 */
interface SkillActions {
  load: (cwd: string) => void
  rescan: (cwd: string) => void
  toggle: (data: ListValue, scope: string, name: string, isDisabled: boolean, cwd: string) => void
}

/** Data actions bound to the state setters (created once per component). */
function createActions({ setData, setLoading, setError, setSaved }: ActionsDeps): SkillActions {
  const applyValue = (value: ListValue): void => {
    setData(value)
    setLoading(false)
  }
  const refreshWith = (fetcher: (cwd: string) => Promise<ListValue>, cwd: string): void => {
    setLoading(true)
    setError(false)
    setSaved(false)
    fetcher(cwd)
  ...
  const load = (cwd: string): void => refreshWith(fetchList, cwd)
  const rescan = (cwd: string): void => refreshWith(rescanCatalog, cwd)
  const save = (scope: string, disabled: string[], cwd: string): void => {
    setSaved(false)
    setError(false)
...
function SkillManagerView(): unknown {
  const [data, setData] = useState<ListValue | null>(null)
      ...
      : error
        ...
        : data === null
          ...
          : createElement(Sections, {
              ...
              onToggle: (scope: string, name: string, isDisabled: boolean) =>
                actions.toggle(data, scope, name, isDisabled, cwdOf(scope)),
            }),
  )
}
...
function Header({ loading, error, saved, onRefresh }: HeaderProps): unknown {
  return createElement(
    'div',
...
function ViewSwitch({ view, hasProject, onSwitch }: ViewSwitchProps): unknown {
  const seg = (scope: string, label: string): unknown =>
    createElement(
      ui.Pill,
      {
...
interface SectionsProps {
  ...
  onToggleUnused: () => void
  onToggle: (scope: string, name: string, isDisabled: boolean) => void
}

/** The toggle section for the current view + diagnostics (collapsible). */
function Sections({ data, view, onToggle, sortBy, unusedOnly, onSort, onToggleUnused }: SectionsProps): unknown {
  ...
  return createElement(
    ...
    createElement(SectionBlock, {
      ...
      onSort,
      onToggleUnused,
      onToggle: (name: string, isDisabled: boolean) => onToggle(projectMode ? 'project' : 'global', name, isDisabled),
    }),
    createElement(DiagnosticsBlock, { diagnostics: data.diagnostics }),
  )
}

/** DiagnosticsBlock props。 */
...
function DiagnosticsBlock({ diagnostics }: DiagnosticsBlockProps): unknown {
...
function projectTitleOf(data: ListValue): string {
...
interface SectionBlockProps {
  ...
  onSort: (key: string) => void
  onToggleUnused: () => void
  onToggle: (name: string, isDisabled: boolean) => void
}
...
function SectionBlock({
  ...
  onToggleUnused,
  onToggle,
}: SectionBlockProps): unknown {
  const usageOf = (name: string): UsageEntryValue | undefined => usage?.[name]
...
function SortControls({ sortBy, unusedOnly, onSort, onToggleUnused }: SortControlsProps): unknown {
  const seg = (key: string, label: string): unknown =>
    createElement(
      ui.Pill,
      {
...
function sortSkills(
  skills: SkillRowValue[],
  sortBy: string,
  usageOf: (name: string) => UsageEntryValue | undefined,
): SkillRowValue[] {
  const list = [...skills]
  if (sortBy === 'count') {
...
function SkillRow({ skill, disabled, onToggle, usage }: SkillRowProps): unknown {
  const usageMeta =
    usage === undefined
      ? strings.usageNever()
      : [
  ...
  const sourceText = isProjectSource(skill.source)
    ? strings.sourceProject(skill.source as string)
    : strings.sourceGlobal(skill.source as string)
  return createElement(
    'div',
...
function Switch({ checked, disabled, label, onToggle }: SwitchProps): unknown {
  return createElement(
    'button',
    {
      type: 'button',
