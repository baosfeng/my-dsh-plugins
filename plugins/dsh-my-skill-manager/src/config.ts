...
/** One scope's config section (only `disabled` is used today). */
export interface ScopeConfig {
  disabled: string[]
}

/** Config document: global + project sections. */
...
function emptyConfig(): ConfigDoc {
  return { global: { disabled: [] }, project: { disabled: [] } }
}

/** Read one config file (missing/corrupt → empty config). */
export async function readConfigFile(file: string): Promise<ConfigDoc> {
  try {
...
export function normalizeConfig(config: unknown): ConfigDoc {
  const result = emptyConfig()
  const disabledOf = (section: ConfigScope): string[] => {
    // 断言仅为通过类型检查：运行时语义与迁移前一致（可选链对任意值安全）。
    const list = (config as ConfigDoc | null | undefined)?.[section]?.disabled
...
export async function writeConfigFile(file: string, config: ConfigDoc): Promise<void> {
...
export async function projectConfigFileOf(cwd: string): Promise<string> {
  const root = await findProjectRoot(cwd)
  return join(root, '.dsh', 'skills.enabled.json')
}

/** Read the project config for a cwd (missing/corrupt → empty config). */
export async function readProjectConfig(cwd: string): Promise<ConfigDoc> {
