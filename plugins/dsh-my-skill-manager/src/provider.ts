...
import type { SkillCatalogEntry, SkillProvider } from './types.js'
...
export function createDisablerProvider(): SkillProvider {
  return {
    name: PROVIDER_NAME,
    async list(options?: { cwd?: string }): Promise<SkillCatalogEntry[]> {
...
export async function disabledNamesOf(cwd?: string): Promise<string[]> {
