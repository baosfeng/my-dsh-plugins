...
- **设置页可视化配置**：「设置 → 插件 → 会话标题生成」可视化编辑全部 8 项配置，保存写回 profile 的 `cordis.patch.yml` 并**热生效**（不必重启），无需手写 yml。
...
![设置 → 插件 → 会话标题生成：8 项配置可视化编辑](./assets/settings-panel.png)

「设置 → 插件 → 会话标题生成」提供 8 项配置的可视化编辑（不写 yml 也能改）：

| 分组 | 项                                                                        |
| ---- | ------------------------------------------------------------------------- |
| 开关 | `enabled`                                                                 |
| 文本 | `template`（多行模板）、`provider`、`model`（后两者留空 = 跟随本次会话）  |
| 数字 | `maxTitleBytes`、`maxInputBytes`、`maxOutputTokens`、`timeoutMs`（正整数） |

- 保存走 `PUT /session-title-gen/api/config`：**先合并该行已有键再写回** profile 的 `cordis.patch.yml`——不会删掉你在同一行手写的其它配置键，也不会动别的插件行。
- 保存**立即生效**：模板 / 上限改动对后续会话生效；关闭 `enabled` 后立刻停止监听会话事件（不必重启 DSH）。
- 非法值只忽略该字段并回退默认值（同一次提交里的其它合法字段照常保存），保存后表单回填**实际生效值**，不会出现「看着存了、其实没生效」。
- 禁用状态下设置页依然可打开——否则没有任何入口把插件重新打开。
