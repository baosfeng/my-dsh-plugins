...
/** 配置生效值容器：apply 与路由共享同一对象，PUT 落盘成功后原地更新 → 热生效。 */
...
export async function persistSettingsConfig(next: SettingsConfig): Promise<void> {
...
async function handleConfigPut(
  ...
  read: () => SettingsConfig,
  write: (next: SettingsConfig) => Promise<void>,
): Promise<void> {
  let next: SettingsConfig | undefined
  try {
    next = normalizeConfigPatch(await readJsonBody(request as unknown as AsyncIterable<string>), read())
  } catch {
    next = undefined
  }
  if (next === undefined) {
...
function mountConfigRoute(
  ...
  scope.effect(() => {
    ...
    try {
      ...
      return () => {
        if (routeDisposers.get(hostCtx) === dispose) routeDisposers.delete(hostCtx)
        dispose()
      }
    } catch (error) {
