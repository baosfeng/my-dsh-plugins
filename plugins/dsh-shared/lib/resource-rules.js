...
export function evaluateResourceAlerts(sample, limits = DEFAULT_RESOURCE_LIMITS) {
    const alerts = [];
 ...
 * CPU/内存超限只告警不降级。
 */
function isCriticalOverLimit(sample, limits = DEFAULT_RESOURCE_LIMITS) {
...
export function shouldEnterDegrade(history, limits = DEFAULT_RESOURCE_LIMITS, confirmCount = DEFAULT_GUARD_CONFIRM_COUNT) {
    return everyRecent(history, confirmCount, (sample) => isCriticalOverLimit(sample, limits));
}
/** 恢复判定（纯函数）：最近 confirmCount 个样本**全部**关键阈值正常。 */
export function shouldExitDegrade(history, limits = DEFAULT_RESOURCE_LIMITS, confirmCount = DEFAULT_GUARD_CONFIRM_COUNT) {
    return everyRecent(history, confirmCount, (sample) => !isCriticalOverLimit(sample, limits));
}
/** 最近 confirmCount 个样本是否**全部**满足 predicate（不足 confirmCount → false，冷启动保护）。 */
function everyRecent(history, confirmCount, predicate) {
