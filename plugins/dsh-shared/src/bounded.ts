...
export const DEFAULT_BOUND_MAP_SIZE = 1000

/** 列表默认上限（兜底值）。 */
export const DEFAULT_BOUND_LIST_SIZE = 1000

/** 校验上限参数：必须为正整数（fail-fast，不静默退化为无界）。 */
function assertMaxSize(maxSize: number, name: string): void {
  if (!Number.isInteger(maxSize) || maxSize < 1) {
    throw new RangeError(name + ': maxSize 必须为正整数，got ' + String(maxSize))
  }
}
...
export class BoundedMap<K, V> extends Map<K, V> {
  ...
  private readonly evictCallback: ((key: K, value: V) => void) | undefined
  private evictedCount = 0

  constructor(options: BoundedMapOptions<K, V> = {}) {
    super()
    const maxSize = options.maxSize ?? DEFAULT_BOUND_MAP_SIZE
    assertMaxSize(maxSize, 'boundedMap')
    this.maxSize = maxSize
    this.policy = options.policy === 'fifo' ? 'fifo' : 'lru'
    this.evictCallback = options.onEvict
    if (options.entries !== undefined) {
      for (const [key, value] of options.entries) this.set(key, value)
    }
  }
  ...
  stats(): BoundedMapStats {
    return { size: this.size, maxSize: this.maxSize, evicted: this.evictedCount }
  }
  ...
  private trim(): void {
    while (super.size > this.maxSize) {
      ...
      const value = super.get(oldest.value) as V
      super.delete(oldest.value)
      this.evictedCount += 1
      this.evictCallback?.(oldest.value, value)
    }
  }
}

/** 创建有界 Map（见 {@link BoundedMap}）。 */
export function boundedMap<K, V>(options: BoundedMapOptions<K, V> = {}): BoundedMap<K, V> {
  return new BoundedMap(options)
}
 ...
 */
export function boundList<T>(options: BoundedListOptions<T> = {}): BoundedList<T> {
  const maxSize = options.maxSize ?? DEFAULT_BOUND_LIST_SIZE
  assertMaxSize(maxSize, 'boundList')
  const buffer: T[] = []
  let evictedCount = 0
  const trim = (): void => {
    if (buffer.length <= maxSize) return
    const drop = buffer.length - maxSize
    buffer.splice(0, drop)
    evictedCount += drop
    options.onEvict?.(drop)
  }
  const pushAll = (items: readonly T[]): void => {
    for (const item of items) buffer.push(item)
    trim()
  }
  if (options.items !== undefined) pushAll(options.items)
  return {
    push(item) {
      buffer.push(item)
      trim()
