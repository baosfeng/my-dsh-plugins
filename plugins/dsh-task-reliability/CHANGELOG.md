...
- fix(task-reliability): #253 补齐 **cucumber 步骤**的落盘就绪判据——`test/features/steps/task-reliability.steps.mjs` 4 处「固定 sleep 后读盘」（`:238` 注册表已写入 / `:308` failed / `:325` done / `:389` resumeAt）改为先 `await this.drainSaves()` 再读盘；World 的 `drainSaves()` 是**两层**就绪信号（`void dispatch(...)` 启动的校验/恢复流程 settle → `shared.drainSaves()` 写链 drain）。根因：#239 只改了 vitest 侧 6 处读盘点，cucumber 侧漏网；且「只等落盘链」覆盖不到「业务流程尚未推进到 `save()`」的窗口（CI run 34732200802：`'checking' !== 'done'`）。新增 `test/cucumber-steps-readiness.mjs`：注入 60ms 慢 IO + 驱动**真实步骤定义**的确定性 RED（修复前 3/3 红，形态与 CI 逐字符一致），并作为常驻防回归
- fix(task-reliability): #233 后续 消除写路径异步化引入的**落盘时序 flaky**——新增 `shared.drainSaves()` **确定性就绪信号**（等待挂起的防抖写 + 写串行链），测试 helper（`taskOf`/`storeOf` 等 6 处「固定 sleep 后读盘」）改为「先 drain 再读盘」；新增 `test/persist-race.mjs`（注入 60ms 慢 IO 的确定性复现 + 防回归）。根因：异步化后「timer + 微任务 + I/O」在 CI 高负载下超过 10/20ms 固定 sleep → 读到旧状态（`'done' !== 'active'`）或文件尚未创建（ENOENT）；纯固定 sleep 不表达条件，只表达「我猜够了」（docs/踩坑/固定sleep等异步落盘导致CI-flaky.md）
