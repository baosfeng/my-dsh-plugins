...
function applyApprovalPolicy(sessionId, enabled, shared) {
    ...
    if (approval === undefined || approval === null || typeof approval.setPolicy !== 'function')
        return;
...
export function applyMode(body, shared) {
    if (typeof body.tracking === 'boolean')
        shared.store.mode.tracking = body.tracking;
    if (typeof body.verify === 'boolean')
        shared.store.mode.verify = body.verify;
    if (typeof body.autopilot === 'boolean')
        shared.store.mode.autopilot = body.autopilot;
...
async function postTrigger(request, response, shared) {
    ...
    if (action === 'register')
        return triggerRegister(body, response, shared);
    if (action === 'mode')
        return triggerMode(body, response, shared);
    if (action === 'answer')
        return triggerAnswer(body, response, shared);
    if (action === 'status')
        return triggerStatus(response, shared);
...
async function dispatchPost(method, request, response, shared) {
    if (method === 'tasks')
        return postTasks(request, response, shared);
    const taskMatch = method?.match(/^tasks\/([^/]+)\/(done|pause|resume|delete)$/);
    if (taskMatch !== null)
        return postTaskAction(taskMatch, request, response, shared);
    const answerMatch = method?.match(/^questions\/([^/]+)\/answer$/);
    if (answerMatch !== null)
        return postAnswer(answerMatch, request, response, shared);
    if (method === 'mode')
        return postMode(request, response, shared);
    if (method === 'config')
        return putConfig(request, response, shared);
    if (method === 'trigger')
        return postTrigger(request, response, shared);
    return false;
}
async function dispatch(method, requestMethod, request, response, shared) {
    ...
    if (requestMethod === 'GET')
        return dispatchGet(method, request, response, shared);
    if (requestMethod === 'POST')
        return dispatchPost(method, request, response, shared);
    if (requestMethod === 'PUT' && method === 'config')
        return putConfig(request, response, shared);
    return false;
}
