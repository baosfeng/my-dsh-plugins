...
const strings = {
    ...
    settingsAutopilotGraceMsHint: () => isZh()
    ...
    settingsRescueOnTruncationHint: () => isZh()
...
function Switch({ label, hint, on, onChange }) {
    return createElement('div', { className: 'dtr-switch-row' }, createElement('div', { className: 'dtr-switch-info' }, createElement('div', { className: 'dtr-switch-label' }, label), createElement('div', { className: 'dtr-switch-hint' }, hint)), createElement('div', {
...
function TaskRow({ task, onAction }) {
    const meta = [];
    if (task.mode === 'verify')
        meta.push(strings.modeVerify());
    if (task.loopCount > 0)
        meta.push(strings.loops(task.loopCount));
    if (task.verifyCount > 0)
        meta.push(strings.verifies(task.verifyCount));
    return createElement('div', { className: 'dtr-task' }, createElement('div', { className: 'dtr-task-head' }, createElement('span', { className: `dtr-badge ${statusClass(task.status)}` }, statusLabel(task.status))), createElement('div', { className: 'dtr-desc' }, task.description), meta.length > 0 ? createElement('div', { className: 'dtr-task-meta' }, meta.join(' · ')) : null, createElement('div', { className: 'dtr-actions' }, task.status !== 'done'
        ? createElement('button', { className: 'dtr-btn', onClick: () => onAction(task.id, 'done') }, strings.done())
        : null, task.status === 'active' || task.status === 'checking'
        ? createElement('button', { className: 'dtr-btn', onClick: () => onAction(task.id, 'pause') }, strings.pause())
        : createElement('button', { className: 'dtr-btn', onClick: () => onAction(task.id, 'resume') }, strings.resume()), createElement('button', { className: 'dtr-btn', onClick: () => onAction(task.id, 'delete') }, strings.delete())));
}
/** 单条待确认问题：已回答显示答案文本，未回答显示输入框 + 回答按钮。 */
function QuestionRow({ question, onAnswer }) {
    const [value, setValue] = useState('');
    if (question.answer !== undefined) {
        return createElement('div', { className: 'dtr-question' }, createElement('div', { className: 'dtr-question-text' }, question.question), createElement('div', { className: 'dtr-answered' }, `${strings.answer()}：${question.answer}`));
    }
    return createElement('div', { className: 'dtr-question' }, createElement('div', { className: 'dtr-question-text' }, question.question), createElement('textarea', {
        className: 'dtr-textarea',
        value,
        placeholder: strings.answer(),
        onChange: (event) => setValue(event.target.value),
    }), createElement('div', { className: 'dtr-actions' }, createElement('button', {
        className: 'dtr-btn',
        onClick: () => {
            if (value.trim() !== '') {
...
function RegisterForm({ onRegister }) {
    ...
    return createElement('div', { className: 'dtr-section' }, createElement('div', { className: 'dtr-section-title' }, strings.register()), createElement('textarea', {
        className: 'dtr-textarea',
        value: desc,
...
function Panel({ scope, visible }) {
    ...
    const load = async () => {
        try {
            ...
            if (infoRes.body?.ok)
                setInfo(infoRes.body.value);
            if (tasksRes.body?.ok)
                setTasks(tasksRes.body.value);
            if (qRes.body?.ok)
                setQuestions(qRes.body.value);
            setLoadError('');
        }
        catch {
    ...
    return createElement('div', { className: 'dtr-panel' }, loadError !== '' ? createElement('div', { className: 'dtr-empty' }, loadError) : null, createElement('div', { className: 'dtr-section' }, createElement(Switch, {
    ...
    })), createElement(RegisterForm, { onRegister: register }), createElement('div', { className: 'dtr-section' }, createElement('div', { className: 'dtr-section-title' }, strings.tasks()), tasks.length === 0
        ? createElement('div', { className: 'dtr-empty' }, strings.noTasks())
        : tasks.map((task) => createElement(TaskRow, { key: task.id, task, onAction: taskAction }))), createElement('div', { className: 'dtr-section' }, createElement('div', { className: 'dtr-section-title' }, strings.questions()), questions.filter((q) => q.answer === undefined).length === 0
...
function SettingsSwitchRow({ label, hint, on, onChange }) {
    return createElement('div', { className: 'dtr-settings-row' }, createElement('div', { className: 'dtr-settings-info' }, createElement('div', { className: 'dtr-settings-label' }, label), createElement('div', { className: 'dtr-settings-hint' }, hint)), createElement('div', {
        className: 'dtr-toggle',
        'data-on': String(on),
        role: 'switch',
        'aria-checked': String(on),
...
function SettingsTextRow({ label, hint, value, onChange, type }) {
    return createElement('div', { className: 'dtr-settings-row' }, createElement('div', { className: 'dtr-settings-info' }, createElement('div', { className: 'dtr-settings-label' }, label), createElement('div', { className: 'dtr-settings-hint' }, hint)), createElement('input', {
...
function settingsSection(section, draft, patch, num) {
    return createElement('div', { className: 'dtr-settings-section' }, createElement('div', { className: 'dtr-settings-title' }, section.title()), ...section.rows.map((row) => settingsRow(row, draft, patch, num)));
}
...
function TaskReliabilitySettingsView() {
    ...
    return createElement('div', { className: 'dtr-settings' }, ...SETTINGS_SECTIONS.map((section) => settingsSection(section, draft, patch, num)), createElement('div', { className: 'dtr-settings-actions' }, createElement('button', { className: 'dtr-btn', onClick: save }, strings.save()), saved ? createElement('span', { className: 'dtr-settings-saved' }, strings.saved()) : null, error ? createElement('span', { className: 'dtr-settings-error' }, strings.saveFailed()) : null));
}
...
function TaskReliabilityTabBody(props) {
...
function TaskReliabilityTabTitle(props) {
    const info = typeof props.useTabInfo === 'function' ? props.useTabInfo() : undefined;
