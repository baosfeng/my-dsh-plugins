...
export const DIRECT_CONTINUE_TEXT = (desc) => `【任务自动继续】你之前的任务尚未确认完成，请继续完成它：${desc}。` +
...
export const AUTOPILOT_DENY_REASON = '【自主决策模式】用户当前不在线，无法回答问题。请基于已有信息和上下文做出最合理的决策并继续执行，' +
    '不要再次询问用户。该问题已记录，用户回来后统一处理。';
/** 超时后迟到回答的提示（issue #145：与「不在线」文案区分，回答不被静默丢弃）。 */
export const AUTOPILOT_LATE_ANSWER_TEXT = (answer) => `【用户已回答】用户已对你刚才的询问提交回答：“${answer}”，已记录到待确认列表。` +
    '若你已按自动决策继续执行，请以用户回答为准调整后续步骤。';
export const RESUME_CONTINUE_TEXT = (desc) => `【系统重启恢复】系统此前在任务执行中被中断（休眠/重启），请继续完成之前的任务：${desc}。` +
    '先回顾当前进度，然后继续执行剩余部分，直到任务完成。';
export const ASK_TIMEOUT_CONTINUE_TEXT = '【用户长时间未响应】你之前询问用户的问题长时间未得到回答，请基于已有信息和上下文自行决策并继续执行任务，' +
    '不要再次询问用户。被跳过的询问已记录，用户回来后统一处理。';
export const WAKE_CONTINUE_TEXT = (desc) => `【系统唤醒恢复】系统此前因锁屏/休眠/网络中断而停滞，请继续完成之前的任务：${desc}。` +
    '先回顾当前进度，然后继续执行剩余部分，直到任务完成。';
export const RESCUE_CONTINUE_TEXT = '【回合输出未完成】你上一回合的输出因达到输出长度上限或异常中断而未完成。' +
