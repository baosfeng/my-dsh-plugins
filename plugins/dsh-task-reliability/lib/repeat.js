 ...
 */
import { REPEAT_BUFFER, REPEAT_CONSECUTIVE, REPEAT_MAX_PER_SESSION, REPEAT_SIM_THRESHOLD, TOOL_LOOP_CONSECUTIVE, TOOL_LOOP_WINDOW, } from './constants.js';
...
export function detectReasoningLoop(segments, opts = {}) {
    ...
    if (segments.length < consecutive + 1)
        return false;
    let streak = 0;
...
export function detectToolCallLoop(toolCalls, opts = {}) {
    ...
    if (n >= consecutive) {
        ...
        if (last.every((call) => sameCall(call, last[0])))
            return true;
    }
    for (let length = 2; length <= window; length++) {
        ...
        if (first.every((call, i) => sameCall(call, second[i])))
            return true;
    }
    return false;
}
...
function handleBlockEnd(buffer, segments, repeatState, opts) {
    ...
    if (!isReasoning && buffer.type !== 'text')
        return;
    ...
    if (text.length < 50)
        return;
    segments.push(text);
    if (segments.length > REPEAT_BUFFER)
        segments.shift();
    if (repeatState.gaveUp)
        return;
    if (!detectReasoningLoop(segments, opts))
        return;
    if (!raiseLoop(repeatState, opts, 'reason'))
        return;
    ...
    return (async function* () {
        for await (const chunk of stream) {
            ...
            if (chunk.type === 'block-start') {
                ...
                if (buffer !== undefined)
                    handleBlockEnd(buffer, segments, repeatState, opts);
            }
