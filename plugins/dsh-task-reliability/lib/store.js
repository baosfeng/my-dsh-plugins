...
function normalizeMode(mode) {
    return {
        ...
        sessionAutopilot: mode.sessionAutopilot !== null && typeof mode.sessionAutopilot === 'object' ? mode.sessionAutopilot : {},
    };
}
function normalizeStore(parsed) {
    ...
    if (parsed.mode !== null && typeof parsed.mode === 'object')
        store.mode = normalizeMode(parsed.mode);
    return store;
}
 ...
 */
export function drainStoreWrites() {
    return writeChain;
}
/**
 ...
 */
export function saveStoreSync(dir, store) {
    const path = join(dir, STORE_FILE);
    const tmp = `${path}.tmp-${process.pid}`;
...
export function registerTask(store, input) {
    ...
    if (sessionId === '' || description === '')
        return { ok: false, error: 'sessionId and description are required' };
    if (description.length > MAX_DESC)
        return { ok: false, error: `description too long (max ${MAX_DESC})` };
    const existing = activeTaskOf(store, sessionId);
    if (existing !== undefined)
        return { ok: false, error: `session already has an active task (${existing.id})` };
    const now = Date.now();
    const task = {
        id: randomId('task'),
        sessionId,
...
export function addQuestion(store, sessionId, question) {
    ...
    if (store.questions.some((q) => q.sessionId === sessionId && q.question === text && q.answer === undefined))
        return;
...
export function answerQuestionByNote(store, sessionId, note, answer) {
    const question = store.questions.find((q) => q.sessionId === sessionId && q.question === note && q.answer === undefined);
    if (question === undefined)
        return { ok: false, error: 'question not found' };
    question.answer = typeof answer === 'string' ? answer.slice(0, 1000) : '';
    question.answeredAt = Date.now();
