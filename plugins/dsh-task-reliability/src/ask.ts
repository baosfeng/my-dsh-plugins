...
import type { AgentLike, AnyObject, ExecLike, NextFn, SharedContext } from './types.js'
...
function firstQuestion(argumentsValue: unknown): AnyObject | undefined {
  const questions = (argumentsValue as AnyObject | null | undefined)?.questions
...
function questionLine(first: AnyObject): string {
 ...
 */
function timeoutDecision(autopilot: boolean, exec: ExecLike, agent: AgentLike, shared: SharedContext): AnyObject {
...
function answerTextOf(value: unknown): string {
  try {
    const answers = (value as AnyObject | null | undefined)?.value?.answers
    if (!Array.isArray(answers)) return ''
    const texts = answers
      .filter((a) => a !== null && typeof a === 'object')
      .map((a) => (Array.isArray(a.selected) ? a.selected.filter((s: unknown) => typeof s === 'string') : []))
 ...
 */
function recordLateAnswer(shared: SharedContext, agent: AgentLike, argumentsValue: unknown, value: unknown): void {
