 ...
 */
export const TOOL_LOOP_READONLY_TOOLS: string[] = [
  'browser_snapshot',
  'browser_eval',
  'browser_tabs',
  'read',
...
export const DIRECT_CONTINUE_TEXT = (desc: string): string =>
  `【任务自动继续】你之前的任务尚未确认完成，请继续完成它：${desc}。` +
  '检查当前进度，列出剩余未完成的部分并逐一执行，直到任务真正完成。' +
  '如果任务实际上已经完成，请明确说明已完成并结束。'

export const REPEAT_BREAK_TEXT = (level: number, kind = 'reason'): string => {
  if (kind === 'tool') {
    return level >= 2
...
export const AUTOPILOT_LATE_ANSWER_TEXT = (answer: string): string =>
  `【用户已回答】用户已对你刚才的询问提交回答：“${answer}”，已记录到待确认列表。` +
  '若你已按自动决策继续执行，请以用户回答为准调整后续步骤。'

export const RESUME_CONTINUE_TEXT = (desc: string): string =>
...
export const WAKE_CONTINUE_TEXT = (desc: string): string =>
