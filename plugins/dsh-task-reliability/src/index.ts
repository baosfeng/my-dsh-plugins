...
export function apply(ctx: DshContext, config: AnyObject | undefined): SharedContext {
  ...
  shared.saveConfig = async (payload: Record<string, unknown>): Promise<void> => {
    const next = buildOptionsFrom(payload)
    await writePatchConfig(
      patchFileOf(currentProfile()),
      'task-reliability',
      configToPlain(next) as unknown as ConfigDict,
    )
...
function strOption(value: unknown, fallback: string): string {
...
function codeSet(value: unknown): Set<string> {
  return Array.isArray(value) && value.length > 0 ? new Set(value as string[]) : RETRYABLE_CODES
}

/** 只读工具集合：默认集合 ∪ 用户追加（issue #153，轮询读取不判循环）。 */
function readonlyToolSet(value: unknown): Set<string> {
...
function buildOptions(config: AnyObject | undefined): ResolvedOptions {
  return buildOptionsFrom(config ?? {})
}

function buildOptionsFrom(c: Record<string, unknown>): ResolvedOptions {
  return {
...
function createShared(ctx: DshContext, options: ResolvedOptions): SharedContext {
...
function scheduleResume(ctx: DshContext, shared: SharedContext): void {
...
function scheduleWatchdog(ctx: DshContext, shared: SharedContext): void {
