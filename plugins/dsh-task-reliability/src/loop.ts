...
import type { AnyObject, ExecLike, RepeatKind, RepeatState, ResolvedOptions, SharedContext } from './types.js'

/** 思考/工具循环检测状态（会话级）。 */
export function repeatStateOf(sessionId: string, shared: SharedContext): RepeatState {
...
export function detectNoProgress(repeat: RepeatState | undefined, options: ResolvedOptions): boolean {
...
function argSummaryOf(exec: ExecLike): string {
  try {
 ...
 */
export function recordToolLoop(sessionId: string, exec: ExecLike, shared: SharedContext): boolean {
  ...
  if (shared.options.toolLoopReadonlyTools.has(exec.name as string)) return false
  repeat.toolCalls.push({ name: exec.name as string, arg: argSummaryOf(exec) })
...
export async function loopNotify(shared: SharedContext, kind: RepeatKind, sessionId: string): Promise<void> {
  ...
  try {
    await fetch(url, {
      ...
      body: JSON.stringify({ type: 'loop', kind, sessionId, message: `检测到死循环（${kind}）` } as AnyObject),
    })
  } catch {
    // 通知通道为可选增强，失败不影响主流程。
  }
