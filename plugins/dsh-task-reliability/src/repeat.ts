...
import type { RepeatKind, RepeatState, ToolCall } from './types.js'

/** 循环检测可覆盖的配置子集（ResolvedOptions 的超集兼容）。 */
 ...
 */
export function detectReasoningLoop(segments: string[], opts: LoopOptions = {}): boolean {
 ...
 */
export function detectToolCallLoop(toolCalls: ToolCall[], opts: LoopOptions = {}): boolean {
...
/** 流式 chunk 的最小结构（llm/stream 的 chunk）。 */
...
/** block 缓冲（block-start 建立、delta 累积、block-end 收尾）。 */
...
function appendDelta(buffers: Map<number, BlockBuffer>, chunk: StreamChunk): void {
  const buffer = buffers.get(chunk.index as number)
  if (buffer !== undefined) buffer.text += chunk.text
}

/** 命中一次循环：计数 + 上限判定；未达上限则打上打断标记返回 true。 */
function raiseLoop(repeatState: RepeatState, opts: LoopOptions, kind: RepeatKind): boolean {
  repeatState.count += 1
...
function handleBlockEnd(buffer: BlockBuffer, segments: string[], repeatState: RepeatState, opts: LoopOptions): void {
  ...
  const error = new Error(`reasoning loop detected (count=${repeatState.count})`) as Error & { code: string }
 ...
 */
export function wrapStreamForLoop(
  ...
  return (async function* () {
    for await (const chunk of stream) {
      ...
      if (chunk.type === 'block-start') {
        buffers.set(chunk.index as number, { type: chunk.blockType, text: '' })
      } else if (chunk.type === 'reasoning-delta' || chunk.type === 'text-delta') {
        appendDelta(buffers, chunk)
      } else if (chunk.type === 'block-end') {
        const buffer = buffers.get(chunk.index as number)
        buffers.delete(chunk.index as number)
