...
import type { AgentLike, AnyObject, RescueState, ResolvedOptions, SharedContext } from './types.js'
...
function tableTruncated(lastLine: string): boolean {
...
function sentenceTruncated(lastLine: string): boolean {
 ...
 * 空文本均不判未完成。
 */
export function isOutputTruncated(text: unknown): boolean {
...
export function todoHasPending(events: unknown): boolean {
  if (!Array.isArray(events)) return false
  for (let i = events.length - 1; i >= 0; i--) {
    const event = events[i] as AnyObject | null
    if (event === null || typeof event !== 'object' || event.type !== 'todo/write') continue
    const todos = event.data?.todos
    if (!Array.isArray(todos)) return false
    return todos.some((todo) => {
      const item = todo as AnyObject | null
...
function rescueStateOf(sessionId: string, shared: SharedContext): RescueState {
...
function rescueAllowed(state: RescueState, options: ResolvedOptions, now = Date.now()): boolean {
...
/** 记录一次救场（计数 + 刷新冷却时间）。 */
function recordRescue(state: RescueState, now = Date.now()): void {
  state.count += 1
 ...
 */
export function rescueTurn(agent: AgentLike, shared: SharedContext): void {
 ...
 */
export function rescueAfterError(agent: AgentLike, shared: SharedContext): void {
...
export function markAgentError(agent: AgentLike, shared: SharedContext): void {
