...
import type { AgentLike, AnyObject, SessionLike } from './types.js'
...
function assistantTextOf(event: AnyObject | undefined): string {
...
function scanEvents(events: AnyObject[]): string {
...
export function sessionEvents(session: SessionLike | undefined): AnyObject[] | undefined {
  return session?.snapshotEvents?.() ?? session?.events
}

/** 会话最后一条 assistant 文本消息（校验 agent 结论读取）。 */
export function lastAssistantText(session: SessionLike | undefined): string {
  try {
...
function eventText(event: AnyObject | undefined): string {
...
export function summarizeSession(session: SessionLike | undefined, desc: string): string {
  const parts: string[] = []
  try {
    const events = sessionEvents(session)
