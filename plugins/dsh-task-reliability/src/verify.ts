...
/** 校验结论（校验 agent 输出的 JSON）。 */
interface Conclusion {
  done: boolean
  reason: string
}

function verifyPrompt(task: Task, summary: string): string {
...
function verifyServiceReady(agents: AgentsService | undefined): agents is AgentsService {
...
async function sessionSummary(ctx: DshContext, agent: AgentLike, task: Task): Promise<string> {
  const sessionQuery = ctx.get<SessionQueryService>('sessionQuery')
  ...
  try {
    ...
    return summarizeSession(log as SessionLike | undefined, task.description)
  } catch {
    return task.description
  }
}

async function spawnVerifier(agents: AgentsService, task: Task, agent: AgentLike): Promise<AgentHandle | undefined> {
  try {
...
async function collectConclusion(handle: AgentHandle, task: Task, summary: string): Promise<Conclusion | undefined> {
  try {
...
async function disposeHandle(handle: AgentHandle): Promise<void> {
  try {
...
function continueWithConclusion(
  task: Task,
  agent: AgentLike,
  save: () => void,
...
/** 校验未完成日志（issue #155）。 */
function logVerifyContinue(ctx: DshContext, task: Task, agent: AgentLike, conclusion: Conclusion | undefined): void {
  ctx.logger?.info?.(
...
function parseConclusion(text: unknown): Conclusion | undefined {
...
function resumeServiceReady(agents: AgentsService | undefined): agents is AgentsService {
  return agents !== undefined && agents !== null && typeof agents.resume === 'function'
}

/** 取回 agent：live 直接复用，否则 resume；失败标记任务 failed。 */
async function tryResumeAgent(agents: AgentsService, task: Task): Promise<AgentLike | undefined> {
  try {
...
function wakeAgent(task: Task, agent: AgentLike): void {
  try {
...
export async function resumeActiveTasks(
  ctx: DshContext,
  store: Store,
  save: () => void,
  emit: EmitFn | undefined,
 ...
 */
export async function wakeStalledTask(
  ctx: DshContext,
  task: Task,
  save: () => void,
  emit: EmitFn | undefined,
