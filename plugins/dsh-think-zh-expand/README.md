...
<div align="center">
  <img alt="思考块渲染 Markdown 与 Mermaid 图表" src="https://unpkg.com/dsh-think-zh-expand/assets/think-markdown.png" width="340" />
</div>

**DSH 思考增强插件**：让 agent 的思考（reasoning）与回复强制使用中文，对话里的思考内容**流式中自动展开、输出完成后自动收起**（点击标题行可展开回看），并把界面残留的硬编码英文（Thinking / Tool Call 等）**中文化**。

## 功能

- **思考强制中文**（server 端）：经 `systemPrompt.section` 注入一条最高优先级的语言规则——思考过程必须简体中文、最终回复跟随用户语言；英文错误信息与英文上下文不改变语言，代码、命令、路径、术语保持原文。
- **思考流式展开 / 完成后收起**（client 端）：覆盖 `conversation.chat.node` 的 `assistant-step` 渲染器——流式生成中自动展开（推理过程可见），输出完成后自动收起为单行摘要，点击标题行可手动展开 / 收起；同样走 Markdown 渲染（代码块 / 表格 / 公式 / **Mermaid 图表**）。
