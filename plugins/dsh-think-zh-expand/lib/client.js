...
exports.DEFAULT_EXPANDED = void 0;
exports.resolveDefaultExpanded = resolveDefaultExpanded;
...
exports.DEFAULT_EXPANDED = true;
/** 模块级生效值：渲染时作为 useState 初值读取。 */
let defaultExpanded = exports.DEFAULT_EXPANDED;
/** 配置快照 → 生效值：只有布尔 defaultExpanded 生效，其余（含 null/字符串）回退 true。 */
function resolveDefaultExpanded(config) {
    if (config === null || config === undefined)
        return exports.DEFAULT_EXPANDED;
    const value = config.defaultExpanded;
    return typeof value === 'boolean' ? value : exports.DEFAULT_EXPANDED;
}
/** 应用一份配置快照，返回生效值。 */
function setDefaultExpanded(config) {
...
function initConfigFromServer() {
    ...
    return fetch(CONFIG_URL)
        ...
        if (payload.value === null || typeof payload.value !== 'object')
            return;
        setDefaultExpanded(payload.value);
    })
        .catch(() => {
