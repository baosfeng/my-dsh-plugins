/**
 * dsh-think-zh-expand — client 端 part 片段的全局类型声明（issue #383 设置页签）。
 *
 * src/client/settings.ts 是 part 片段（无 import/export → TypeScript script 模式 →
 * 全局作用域），它引用的 React API、共享样式注入器与 CommonJS 变量都由
 * __ModuleLoader__ factory 作用域在运行时提供（模板 lib/client.src.js 里
 * `const { createElement, useState, useEffect } = require('react')` + dsh-shared
 * client-parts 注入），这里补上这些运行时变量的类型契约。
 *
 * 本文件只被 tsconfig.client.json 加载（插件根 tsconfig.json 已 exclude
 * src/client），不污染仓库级类型环境。
 */
