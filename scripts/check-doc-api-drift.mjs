 ...
 * 得出的派生文档**——它声明本仓库实际调用哪些宿主能力、哪些事件、哪 6 个 UI 槽位、哪些 API
 * 代码零使用。这类内容靠人工维护必然漂移，而漂移已经造成过两次真实事故：
 *   ① skill 里长期主推第三方 ctx.betterSidebar，而仓库 19 个插件实测 0 使用；
 *   ② 文档写 ctx.config，而**该 API 根本不存在**（11 处出现全在注释里；配置入口是
