 ...
 *
 * 阈值依据（issue #188 用回放统计修正，见 scripts/analyze-impact-replay.mjs）：
 *   · 只按「直接依赖方数量 ≥ 3」判定的旧规则会把**传递闭包很小**的高扇入插件也顶成全量
 *     （回放统计里 dsh-md-render 就是这种形态）；
 *   · 现规则 = 先算传递闭包（依赖方 + 依赖方的依赖方 …），闭包超过一半插件才全量。
