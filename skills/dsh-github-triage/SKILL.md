...
- 验收：修复代码 + 本地测试通过（npm test）+ **改动文件的 `npx prettier --check <files>` exit 0** + PR 已创建；逐项自查见 references/dispatch-playbook.md 的「验收标准」，格式判据本体见 `docs/开发指南/leader行为规范.md` 第七节
...
2. **验收核查**：每个子任务完成后，对照 references/dispatch-playbook.md 的「验收标准」表**独立核查**，不只信子 agent 自报：
   ...
   - 本地测试 + 格式：抽查子 agent 报告的关键测试输出与**改动文件的 `npx prettier --check <files>` exit 0** 证据，**只抽检最关键 1~2 条断言**；要完整重跑 / 多轮取证时**派发**（例外须当轮声明，见 `docs/开发指南/leader行为规范.md` 第四节）
...
| 子 agent 自报完成即推进                                       | 主 agent 必须按验收标准独立核查（变更范围/本地测试/格式/CI），未达标退回修或重派                                                                                                                                                                                                                                                                                            |
...
| 验收不达标就提交                                              | 按验收标准逐项自查：根因/修复/测试/格式/PR 缺一不可（格式 = 改动文件的 `npx prettier --check <files>` exit 0）                                                                                                                                                                                                                                                                                                                         |
