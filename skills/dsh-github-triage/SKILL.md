...
- **压缩阈值必须调低**：`compaction-basic` 的 `thresholdRatio` 默认 `0.8`，而模型 `contextWindow` 是 100 万 → **上下文涨到 80 万才压缩**，在那之前每步全量重放。实测单会话跑到 47 万上下文 × 503 步 = **1.38 亿 token**，且全程零压缩事件。派发前在 profile 的 `cordis.patch.yml` 里按 `compaction-basic` 的 id 把该值压到 `0.15~0.25`。
...
| 档位 | 典型形态                             | 累计 token 上限 | 步数上限 |
| ---- | ------------------------------------ | --------------- | -------- |
| 简单 | 单文件小改 / 加一条检测 / 改一处配置 | ≤ 150 万        | ≤ 40     |
| 中等 | 多文件 + 配套测试                    | ≤ 300 万        | ≤ 70     |
| 复杂 | 跨模块 / 跨仓库 / 大量测试           | ≤ 500 万        | ≤ 100    |

- **派发前（缺一不许派）**：prompt 必须写明 ①「预算：≤ N 步 / ≤ M token」；②「到点先交**阶段性成果**并回报卡点，不要继续跑」；③「只回传**结论与证据**，不回传 diff / 日志 / 全量输出」。任务范围要能落进上表某一档，落不进就先拆。
...
| Actions 失败       | 先 `ghops actions logs baosfeng/my-dsh-plugins <run-id> --jobs` 拿**全量 job 清单**（不要从日志内容反推有哪些 job——旧版就是这么漏掉失败 job 的，见 `docs/踩坑/README.md`）→ `--job "<失败 job 名>"` 精确取正文，stderr 的「日志已获取 M/N」会如实报告缺口；单个 job 取不到时按提示走替代路径（网页 job URL）→ 定位失败步骤与根因 → 修复 + 已建 PR；CI 重跑通过（重跑仍失败则继续排查，不得标记完成） |
