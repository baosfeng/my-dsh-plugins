...
- **观测口径**：会话落盘 `~/.dsh/sessions/<cwd-转义>/<session-id>/session.v3.jsonl.zstd`，`zstd -d -c` 解压后读 `assistant/message` 事件的 `usage`（`inputTokens`/`cacheReadTokens`/`outputTokens`）。**子 agent 是独立会话文件，统计成本必须递归 `subagent/catalog` 的 `childId`**，否则严重低估。
