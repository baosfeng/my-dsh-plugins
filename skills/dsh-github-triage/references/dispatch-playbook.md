...
- **上下文预算：leader 与子 agent 不同档，不要一刀切**：**leader 会话的收尾上限可用到 contextWindow 的 60%**（约 60 万）—— 它承担拆解/验收/汇总，上下文天然偏大；**但 50% 起即触发自执行熔断**（只做判读与决策，自执行的调查类工作一律派发，见 `docs/开发指南/leader行为规范.md` 第三节），**不要用"收尾"去卡它**；**子 agent 上限 30%**（约 30 万），到点先固化成果再收尾。注意上下文是**安全边界**，不是成本目标 —— 成本主力始终是**步数**，因为「步数 × 上下文」才是乘法。
...
- **派发前（缺一不许派）**：prompt 必须写明 ①「预算：≤ N 步 / 上下文 ≤ X%」；②「到点先交**阶段性成果**并回报卡点，不要继续跑」；③「只回传**结论与证据**，不回传 diff / 日志 / 全量输出」；④**改动文件的任务**（代码 / 测试 / 文档 / 配置）：「验收判据含格式检查——对改动过的文件跑 `npx prettier --check <files>` 并**贴出 exit 0 的输出**」（`format` 是 CI 阻断项、最高频的红；判据与反模式见 [leader 行为规范](../../../docs/开发指南/leader行为规范.md) 第七节）。任务范围要能落进上表某一档，落不进就先拆。
...
- PR 描述含：**根因、修复内容、验证结果**（测试命令 + 关键输出，含 `npx prettier --check <files>` 的 exit 0）；标题遵循仓库提交规范（Conventional Commits，见 .reasonix/skills/commit-standards），格式 `fix(scope): #<编号> 简述`
