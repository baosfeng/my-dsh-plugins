...
| 坑                         | 现象                                                                                                                | 解决                                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **settings slot 形状变化** | 照旧文档写 `settings.plugin.item` / `ctx.settings.installSection` → 宿主**已无这两者**（零消费者），注册后不出现任何 UI、也不报错 | 列表席位 `settings.plugins.tab` 必填 `id`（本仓库用法）；插件页细粒度席位是 `plugins.item`（list/root，必填 `id`）；整页式需求走分区 slot `settings.section`（list/root，必填 `id`）；席位名与必填字段一律以宿主 `slot-catalog.ts` 为准 |
...
| 载体                | 适用                        | 机制                                                                                                                                                                                                                                                                     |
| ------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 宿主 Slot（设置页） | 设置页开关 / 卡片           | 本仓库：client 半 `ctx.slots.inject('settings.plugins.tab', () => ctx.slots.register({ name: 'settings.plugins.tab', id, order, label }, C))` + localStorage；官方另有插件页席位 `plugins.item` / `plugins.detail.*`、分区 `settings.section`（均 list/root，必填 `id`） |
