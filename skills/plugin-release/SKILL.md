...
- `--push`：全部 tag 创建完 → **逐个推送**（一次 push 只带一个 ref；一次推 N 个 tag 会被 GitHub 合并/丢弃 push 事件，实测零触发）→ 逐个**确认触发** → **并发**等全部 GitHub Release（原来是每个插件串行等 ~55s）。npm 发布状态**不由本地判定**（本机 npm 查询走镜像且缓存陈旧，会报假警报），状态查 npm 包页。固化在 `scripts/lib/release-tag-push.mjs`、`scripts/lib/post-release.mjs`；
