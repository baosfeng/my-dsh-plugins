...
Two surfaces have no target list of their own and are routinely left uncovered: **protocol** (JSON-RPC / SDK / gateway / ACP / hooks / error codes / HTTP routes — playbook surfaces 2 and 4) and **asset/config schema** (per-package `config.*` schemas, which export-diff and import-graph scans cannot see). An audit that does not read them must declare them unverified (未核验) in the report — never default an unread surface to safe.
...
| 5   | Session data        | `packages/session/**`, `packages/session-query/**`, `packages/storage/**`, `packages/core/session`, `session-format-guard.expected.e2e.ts`: the format guards, migrate vs refuse (read the guard code, not the README), JSONL encoding, projection/query/export surface                                                                   | lib constants and `resources/sql/` of `dsh-session` + supplement packages `dsh-storage-sqlite`, `dsh-session-query*`, `dsh-session-log-export` (a `from` side older than the storage/session-query split publishes a since-removed standalone SQLite persistence package instead — take its name from that version's manifest) |
...
<Name every surface this pass did NOT read. At minimum: the protocol surface (JSON-RPC / SDK / gateway / ACP / hooks / error codes / HTTP routes) and the asset/config-schema surface (per-package `config` schemas — e.g. `@deepseek-ai/dsh-persona` now takes `prefix` (required) / `suffix` / `complete` / `includeRuntimeContext` while the old `config.text` is gone; a removed config key breaks config authors and shows up in no export diff). An unlisted surface reads as "unchanged": never default an unread surface to safe.>
