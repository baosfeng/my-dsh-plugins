...
- **Field note** (2026-08-28, dsh-mnemon rc.2/alpha.1 dual cohort): Migrating custom RPC registrations to alpha.1's two-argument `handle` shape passed alpha-side mocks but regressed the still-installable rc.2 Host, whose real service reads `options.authority`. The verified branch-free seam always supplies the legacy authority object: rc.2 consumes it and alpha.1 ignores the trailing JavaScript argument, while the retained `remoteAccess` setting continues to protect rc.2 management channels. See [dsh-mnemon PR #105](https://github.com/omdsh-dev/dsh-mnemon/pull/105) and the frozen-skill [H11 benchmark task](https://github.com/oh-my-dsh/dsh-plugin-upgrade-skill/blob/main/benchmark/tasks/H11-dual-cohort-rpc/README.md) (the upstream `examples/04-dual-cohort-plugin.md` fixture is not shipped in this trimmed copy).
...
| `IWorkspaces` / `ctx.workspaces.connectWorkspace` / `pickDirectory` / `startSession` | List and Workspace CRUD stay on `@deepseek-ai/dsh-api-workspace-controller/client`; navigation and directory UI moved to `ctx.uiWorkspace` (`@deepseek-ai/dsh-client-ui-workspace/client`). Snapshot fields `baselinesReady` / `recentWorkspaceId` are gone — see [DSH-0.1.2-A1-32](#dsh-012-a1-32--iworkspaces-navigation-methods-moved-to-ctxuiworkspace) |
...
- **Migration recipe**: Keep list and Workspace CRUD on `ctx.workspaces` (`IWorkspaces` from `@deepseek-ai/dsh-api-workspace-controller/client`). Move navigation and directory UI to `ctx.uiWorkspace` (`UiWorkspace` from `@deepseek-ai/dsh-client-ui-workspace/client`):
  ...
  - If the source names `IWorkspaces` / `UiWorkspace`, those owning packages must be direct type/dev/peer dependencies ([DSH-0.1.2-A1-25](#dsh-012-a1-25--deepseek-aidsh-client-runtime-package-removed-client-symbols-migrated-by-domain)).
