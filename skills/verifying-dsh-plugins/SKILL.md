...
- **重跑清单是幂等的（issue #329 保证，放心重跑）**：目标清单已存在时**绝不整文件重写** ——
  ...
  人工追加的 `## 验证记录` 段**逐字节保留**、已勾选的 `[x]` 与未勾选的 `[ ]` 都按原样留存；
  脚本只刷新自己拥有的「自动验证项」。所以复验 / 换端口复现 / 排查 flaky 都可以随时重跑，**不会丢证据**；
  确需把本轮环境写进头部时加 `--refresh-header`（默认关，issue #329）。
...
- **`## 验证记录` 段是你自己的地盘（issue #329）**：脚本重跑只动「自动验证项」，人工段与勾选**逐字节不变**
